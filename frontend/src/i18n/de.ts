export default {
  common: {
    save: 'Speichern',
    cancel: 'Abbrechen',
    delete: 'Löschen',
    edit: 'Bearbeiten',
    create: 'Erstellen',
    back: 'Zurück',
    loading: 'Laden...',
    loadingTimeout: 'Das Laden dauert länger als erwartet.',
    retry: 'Erneut versuchen',
    noData: 'Keine Daten vorhanden',
    confirm: 'Bestätigen',
    search: 'Suchen',
    changeAvatar: 'Profilbild ändern',
    filter: 'Filtern',
    actions: 'Aktionen',
    yes: 'Ja',
    no: 'Nein',
    close: 'Schließen',
    add: 'Hinzufügen',
    noResults: 'Keine Ergebnisse',
    active: 'Aktiv',
    inactive: 'Inaktiv',
    previous: 'Zurück',
    next: 'Weiter',
    name: 'Name',
    description: 'Beschreibung',
    status: 'Status',
    createdBy: 'Erstellt von',
    slots: 'Plätze',
    skipToContent: 'Zum Inhalt springen',
    uploadAvatar: 'Avatar hochladen',
    removeAvatar: 'Avatar entfernen',
    avatarUploaded: 'Avatar hochgeladen',
    avatarRemoved: 'Avatar entfernt',
    confirmDeleteTitle: 'Löschen bestätigen',
    confirmDeleteMessage: 'Möchten Sie diesen Eintrag wirklich löschen? Dies kann nicht rückgängig gemacht werden.',
    error: 'Fehler',
    errorGeneric: 'Ein unerwarteter Fehler ist aufgetreten',
    success: 'Erfolg',
    created: 'Erstellt',
    deleted: 'Gelöscht',
    view: 'Anzeigen',
    none: 'Keine',
  },
  nav: {
    dashboard: 'Dashboard',
    rooms: 'Räume',
    family: 'Familie',
    messages: 'Nachrichten',
    jobs: 'Jobbörse',
    cleaning: 'Putz-Orga',
    calendar: 'Kalender',
    forms: 'Formulare',
    fundgrube: 'Fundgrube',
    bookmarks: 'Lesezeichen',
    parentLetters: 'Elternbriefe',
    profile: 'Profil',
    admin: 'Verwaltung',
    logout: 'Abmelden',
    discover: 'Entdecken',
    more: 'Mehr',
    mainNavigation: 'Hauptnavigation',
    directory: 'Verzeichnis',
  },
  auth: {
    login: 'Anmelden',
    register: 'Registrieren',
    email: 'E-Mail',
    password: 'Passwort',
    firstName: 'Vorname',
    lastName: 'Nachname',
    phone: 'Telefon',
    forgotPassword: 'Passwort vergessen?',
    or: 'oder',
    loginSso: 'Mit SSO anmelden',
    noAccount: 'Noch kein Konto?',
    hasAccount: 'Bereits ein Konto?',
    loginError: 'Anmeldung fehlgeschlagen. Bitte überprüfen Sie Ihre Zugangsdaten.',
    registerError: 'Registrierung fehlgeschlagen.',
    pendingApproval: 'Ihr Konto wartet auf Freischaltung durch einen Administrator.',
    pendingApprovalSuccess: 'Registrierung erfolgreich! Ihr Konto muss zunächst von einem Administrator freigeschaltet werden. Bitte haben Sie etwas Geduld.',
    backToLogin: 'Zurück zur Anmeldung',
    acceptTermsLabel: 'Ich akzeptiere die',
    termsLink: 'Nutzungsbedingungen',
    termsRequired: 'Sie muessen den Nutzungsbedingungen zustimmen.',
    impersonation: {
      banner: 'Angemeldet als {name} (Impersonation)',
      stop: 'Zurück zum Admin',
      toggle: 'Benutzerwechsel (Impersonation)',
      toggleDescription: 'Ermöglicht dem SuperAdmin, sich als anderer Benutzer anzumelden. Nur für Test- und Supportzwecke.',
      dangerWarning: 'Sicherheitsrelevante Funktion — nur aktivieren wenn benötigt',
      loginAs: 'Als User anmelden',
    },
  },
  dashboard: {
    welcome: 'Willkommen, {name}!',
  },
  rooms: {
    title: 'Meine Räume',
    allRooms: 'Alle Räume',
    create: 'Raum erstellen',
    name: 'Name',
    description: 'Beschreibung',
    type: 'Typ',
    section: 'Schulbereich',
    members: 'Mitglieder',
    settings: {
      tabTitle: 'Einstellungen',
      title: 'Raum-Einstellungen',
      modules: 'Module',
      chatHint: 'Aktiviert den Chat-Bereich in diesem Raum',
      filesHint: 'Aktiviert die Dateiablage in diesem Raum',
      parentSpace: 'Elternbereich',
      parentSpaceHint: 'Separater Bereich nur fuer Eltern',
      access: 'Zugang',
      visibility: 'Sichtbarkeit',
      visibilityMembersOnly: 'Nur Mitglieder',
      visibilitySection: 'Schulbereich',
      visibilityAll: 'Alle',
      discussionMode: 'Diskussionsmodus',
      discussionFull: 'Vollstaendig',
      discussionAnnouncementsOnly: 'Nur Ankuendigungen',
      discussionDisabled: 'Deaktiviert',
      allowThreadCreation: 'Mitglieder duerfen Themen erstellen',
      allowThreadCreationHint: 'Wenn deaktiviert, koennen nur Leitung und Admins Diskussionsthemen erstellen',
      childDiscussion: 'Kinder-Diskussion',
      childDiscussionHint: 'Erlaubt Schueler:innen die Teilnahme an Diskussionen',
      saved: 'Einstellungen gespeichert',
    },
    noRooms: 'Sie sind noch keinem Raum zugeordnet.',
    infoBoard: 'Info-Board',
    noPosts: 'Noch keine Beiträge in diesem Raum',
    noMembers: 'Noch keine Mitglieder',
    noSection: 'Kein Bereich',
    publicDescription: 'Öffentliche Beschreibung',
    publicDescPlaceholder: 'Für Nicht-Mitglieder sichtbar...',
    notMember: 'Kein Mitglied',
    joinToSee: 'Beitreten um Inhalte zu sehen',
    types: {
      KLASSE: 'Klasse',
      GRUPPE: 'Gruppe',
      PROJEKT: 'Projekt',
      INTEREST: 'Interessengruppe',
      CUSTOM: 'Sonstige',
    },
    roles: {
      LEADER: 'Leitung',
      MEMBER: 'Mitglied',
      PARENT_MEMBER: 'Eltern-Mitglied',
      GUEST: 'Gast',
    },
    teachers: 'Lehrkr\u00e4fte',
    students: 'Schüler',
    families: 'Familien',
    otherMembers: 'Weitere Mitglieder',
    closedRooms: 'Weitere R\u00e4ume',
    requestJoin: 'Beitritt anfragen',
    joinRequestSent: 'Anfrage gesendet',
    joinRequestMessage: 'Beitrittsanfrage für {room}',
    joinRequestPlaceholder: 'Nachricht an die Leitung (optional)...',
    pendingRequests: 'Offene Anfragen',
    approve: 'Annehmen',
    deny: 'Ablehnen',
    requestApproved: 'Anfrage angenommen',
    requestDenied: 'Anfrage abgelehnt',
    addFamily: 'Familie aufnehmen',
    familyAdded: 'Familienmitglieder hinzugefügt',
    selectFamily: 'Familie auswählen',
    noFamilies: 'Keine Familien vorhanden.',
    addMember: 'Mitglied hinzufügen',
    addTeacher: 'Lehrkraft hinzufügen',
    searchMember: 'Person suchen',
    searchMemberPlaceholder: 'Name oder E-Mail eingeben...',
    memberAdded: 'Mitglied hinzugefügt',
    memberRemoved: 'Mitglied entfernt',
    alreadyMember: 'Ist bereits Mitglied',
    roleChanged: 'Rolle geändert',
    confirmRemoveMember: 'Mitglied wirklich entfernen?',
    filterAll: 'Alle Rollen',
    filterTeacher: 'Lehrkraft',
    filterParent: 'Elternteil',
    filterStudent: 'Schüler/in',
    filterFamily: 'Familie',
    inviteOnly: 'Nur auf Einladung',
    muteFeed: 'Feed stummschalten',
    unmuteFeed: 'Stummschaltung aufheben',
    muted: 'Feed stummgeschaltet',
    unmuted: 'Stummschaltung aufgehoben',
    joinPolicies: {
      OPEN: 'Offen',
      REQUEST: 'Auf Anfrage',
      INVITE_ONLY: 'Nur auf Einladung',
    },
    discussionModes: {
      FULL: 'Voll (alle können antworten)',
      ANNOUNCEMENTS_ONLY: 'Nur Ankündigungen',
      DISABLED: 'Deaktiviert',
    },
    migration: {
      button: 'Klassenwechsel',
      title: 'Klassenwechsel / Migration',
      selectChildren: 'Wählen Sie die Kinder aus, die wechseln sollen:',
      noStudents: 'Keine Schüler:innen in diesem Raum.',
      chooseAction: 'Was soll mit den ausgewählten Kindern geschehen?',
      moveToClass: 'In andere Klasse verschieben',
      moveDesc: 'Die Kinder werden in eine andere Klasse verschoben. Deren Eltern werden automatisch hinzugefügt.',
      leaveSchool: 'Schule verlassen',
      leaveDesc: 'Die Kinder und deren Familien verlassen alle Räume. Die Familie wird deaktiviert.',
      selectTarget: 'Zielklasse auswählen:',
      selectTargetPlaceholder: 'Klasse auswählen...',
      leaveWarning: 'Achtung: Diese Aktion entfernt die ausgewählten Kinder und deren Familien aus allen Räumen und deaktiviert die Familien.',
      leaveConfirm: 'Die folgenden {count} Kind(er) werden die Schule verlassen:',
      summary: 'Zusammenfassung',
      selectedCount: '{count} Kind(er) ausgewählt',
      actionLabel: 'Aktion',
      moveToTarget: 'Verschieben nach: {name}',
      leaveIrreversible: 'Diese Aktion kann nicht rückgängig gemacht werden.',
      execute: 'Ausführen',
      moveSuccess: '{count} Kind(er) erfolgreich verschoben.',
      leaveSuccess: '{count} Kind(er) haben die Schule verlassen.',
    },
  },
  family: {
    parent: 'Elternteil',
    child: 'Kind',
    title: 'Familienverbund',
    create: 'Familienverbund erstellen',
    join: 'Beitreten',
    inviteCode: 'Einladungscode',
    generateCode: 'Code generieren',
    addChild: 'Kind hinzufügen',
    members: 'Familienmitglieder',
    noFamily: 'Sie gehören noch keinem Familienverbund an.',
    name: 'Familienname',
    hours: 'Stundenkonto',
    hoursUnit: 'Stunden',
    jobHours: 'Elternstunden',
    cleaningHours: 'Putzstunden',
    cleaningProgress: 'Putzstunden-Fortschritt',
    confirmed: 'Bestätigt',
    pending: 'Ausstehend',
    remaining: 'Verbleibend',
    totalProgress: 'Gesamtstunden',
    clickForJobs: 'Klicken für geleistete Jobs',
    completedJobs: 'Geleistete Jobs',
    noCompletedJobs: 'Noch keine abgeschlossenen Jobs',
    roles: {
      PARENT: 'Elternteil',
      CHILD: 'Kind',
    },
    inviteMember: 'Mitglied einladen',
    searchMember: 'Mitglied suchen',
    sendInvitation: 'Einladung senden',
    invitationSent: 'Einladung gesendet',
    pendingInvitations: 'Offene Einladungen',
    acceptInvitation: 'Annehmen',
    declineInvitation: 'Ablehnen',
    invitationAccepted: 'Einladung angenommen',
    invitationDeclined: 'Einladung abgelehnt',
    invitedBy: 'Eingeladen von {name}',
    sentInvitations: 'Gesendete Einladungen',
    calendar: 'Familienkalender',
    calendarDesc: 'Termine, zu denen Familienmitglieder zugesagt haben',
    noCalendarEvents: 'Keine anstehenden Termine',
    downloadIcal: 'iCal herunterladen',
    upcomingEvents: 'Anstehende Termine',
    leave: 'Verlassen',
    leaveConfirmTitle: 'Familie verlassen',
    leaveConfirmMessage: 'Möchten Sie diesen Familienverbund wirklich verlassen?',
    leftFamily: 'Familienverbund verlassen',
    soleCustody: 'Alleinsorgeberechtigt',
    soleCustodyHint: 'Halbiert die Pflichtstunden der Familie. Muss von einem Administrator genehmigt werden.',
    soleCustodyRequested: 'Alleinsorgeberechtigung beantragt',
    soleCustodyApproved: 'Alleinsorgeberechtigung genehmigt',
    soleCustodyPending: 'Warte auf Genehmigung',
    soleCustodyApprovedLabel: 'Genehmigt',
    approveSoleCustody: 'Genehmigen',
    noHoursYet: 'Noch keine Stunden geleistet',
    goToJobboard: 'Zur Jobbörse',
  },
  admin: {
    title: 'Verwaltung',
    dashboard: {
      title: 'Dashboard',
      users: 'Benutzer',
      usersDesc: 'Benutzer verwalten, Rollen zuweisen',
      rooms: 'Räume',
      roomsDesc: 'Räume erstellen und verwalten',
      sections: 'Schulbereiche',
      sectionsDesc: 'Krippe, KiGa, GS, MS, OS',
      modules: 'Module',
      modulesDesc: 'Module aktivieren/deaktivieren',
      families: 'Familien',
      familiesDesc: 'Familienverbunde verwalten',
      billing: 'Jahresabrechnung',
      billingDesc: 'Elternstunden-Abrechnung nach Perioden',
      theme: 'Design & Einstellungen',
      themeDesc: 'Farben, Logo, Stunden und Ferien konfigurieren',
      settings: 'Einstellungen',
      settingsDesc: 'Sprache, Registrierung und Jobbörse konfigurieren',
      icalDesc: 'Externe Kalender per iCal-URL abonnieren',
      profileFields: 'Profilfelder',
      profileFieldsDesc: 'Benutzerdefinierte Profilfelder verwalten',
    },
    users: 'Benutzer',
    rooms: 'Räume',
    sections: 'Schulbereiche',
    modules: 'Module',
    config: 'Konfiguration',
    auditLog: 'Protokoll',
    jobReport: 'Stundenbericht',
    familyReport: 'Familien-Stundenbericht',
    jobHoursCol: 'Elternstunden',
    cleaningHoursCol: 'Putzstunden',
    themeTitle: 'Design & Branding',
    themeSaved: 'Design gespeichert',
    schoolInfo: 'Infos zur Schule',
    schoolFullName: 'Vollstaendiger Schulname',
    schoolFullNamePlaceholder: 'z.B. Montessori-Schule Musterstadt e.V.',
    schoolAddress: 'Anschrift',
    schoolAddressPlaceholder: 'Musterstr. 1, 12345 Musterstadt',
    schoolPrincipal: 'Schulleitung',
    schoolPrincipalPlaceholder: 'z.B. Dr. Maria Muster',
    techContactName: 'Technischer Ansprechpartner',
    techContactNamePlaceholder: 'z.B. Max Mustermann',
    techContactEmail: 'E-Mail techn. Ansprechpartner',
    techContactEmailPlaceholder: "z.B. technik{'@'}schule.de",
    schoolInfoHint: 'Diese Informationen werden in der Datenschutzerklaerung und den Nutzungsbedingungen verwendet.',
    schoolInfoSaved: 'Schulinformationen gespeichert',
    logo: 'Logo',
    logoUploaded: 'Logo hochgeladen',
    colorScheme: 'Farbschema',
    preview: 'Vorschau',
    createSection: 'Bereich anlegen',
    sectionName: 'Name',
    sectionDesc: 'Beschreibung',
    sectionOrder: 'Reihenfolge',
    csvExport: 'CSV Export',
    pdfExport: 'PDF Export',
    openJobs: 'Offene Jobs',
    activeJobs: 'Aktive Jobs',
    completed: 'Abgeschlossen',
    trafficLight: {
      green: 'Grün',
      yellow: 'Gelb',
      red: 'Rot',
    },
    familyCol: 'Familie',
    progressCol: 'Fortschritt',
    pendingCol: 'Ausstehend',
    remainingCol: 'Verbleibend',
    trafficLightCol: 'Ampel',
    noSection: 'Kein Bereich',
    bundesland: 'Bundesland',
    bundeslandHint: 'Bestimmt die gesetzlichen Feiertage',
    schoolVacations: 'Schulferien',
    holidaysAndVacations: 'Schulferien & Feiertage',
    addVacation: 'Ferienzeit hinzufügen',
    loadVacations: 'Ferien für Bundesland laden',
    loadVacationsHint: 'Lädt die offiziellen Schulferien 2025/2026 für das gewählte Bundesland',
    removeVacation: 'Entfernen',
    vacationName: 'Bezeichnung',
    vacationFrom: 'Von',
    vacationTo: 'Bis',
    vacationsSaved: 'Ferien & Bundesland gespeichert',
    hoursConfig: 'Stundenregelung',
    totalHoursTarget: 'Gesamtstunden pro Familie/Jahr',
    cleaningHoursTarget: 'davon Putzstunden',
    requireConfirmation: 'Bestätigung durch Verantwortliche erforderlich',
    requireConfirmationHint: 'Wenn deaktiviert, werden Stunden beim Abschließen automatisch gutgeschrieben.',
    saveHoursConfig: 'Stundenregelung speichern',
    hoursConfigSaved: 'Stundenregelung gespeichert',
    maintenance: {
      title: 'Wartungsmodus',
      enabled: 'Wartungsmodus aktivieren',
      warning: 'Nur Administratoren können auf das System zugreifen, wenn der Wartungsmodus aktiv ist.',
      message: 'Wartungsnachricht',
      messagePlaceholder: 'z.B. Systemwartung bis ca. 18:00 Uhr',
      saved: 'Wartungsmodus aktualisiert',
    },
    clamav: {
      title: 'Virenscanner',
      enabled: 'ClamAV aktivieren',
      host: 'ClamAV Host',
      port: 'ClamAV Port',
      hint: 'ClamAV muss als Docker-Container laufen (docker compose --profile clamav up)',
      saved: 'Virenscanner-Einstellungen gespeichert',
    },
    jitsi: {
      title: 'Jitsi Videokonferenz',
      enabled: 'Jitsi aktivieren',
      serverUrl: 'Jitsi-Server-URL',
      hint: 'Standard: meet.jit.si (kostenlos). Eigener Jitsi-Server empfohlen fuer Datenschutz.',
      saved: 'Jitsi-Einstellungen gespeichert',
      joinMeeting: 'Besprechung beitreten',
      startVideoChat: 'Videochat starten',
      addVideoConference: 'Videokonferenz hinzufuegen',
      removeVideoConference: 'Videokonferenz entfernen',
      videoConference: 'Videokonferenz',
      generatingLink: 'Link wird erstellt...',
    },
    settings: {
      title: 'Einstellungen',
      groups: {
        general: 'Allgemein',
        communication: 'Kommunikation',
        integration: 'Integrationen',
        security: 'Sicherheit',
        hours: 'Stunden',
      },
      language: 'Sprache',
      multilanguageEnabled: 'Mehrsprachigkeit aktivieren',
      multilanguageHint: 'Wenn deaktiviert, wird nur die Standardsprache verwendet und der Sprachwechsler ausgeblendet.',
      defaultLanguage: 'Standardsprache',
      availableLanguages: 'Verfügbare Sprachen',
      availableLanguagesHint: 'Wähle die Sprachen, die Benutzer im Sprachwechsler auswählen können. Die Standardsprache ist immer verfügbar.',
      registration: 'Registrierung',
      requireUserApproval: 'Neue Benutzer müssen freigeschaltet werden',
      requireUserApprovalHint: 'Wenn deaktiviert, sind neue Benutzer nach der Registrierung sofort aktiv.',
      jobboard: 'Jobbörse',
      directory: 'Benutzerverzeichnis',
      directoryAdminOnly: 'Verzeichnis nur für Admins sichtbar',
      directoryAdminOnlyHint: 'Wenn aktiviert, können nur Administratoren das Benutzerverzeichnis sehen.',
      saved: 'Einstellungen gespeichert',
      communication: 'Kommunikation',
      parentToParentMessaging: 'Nachrichten zwischen Eltern',
      parentToParentMessagingHint: 'Erlaubt Eltern, sich gegenseitig Nachrichten zu senden.',
      studentToStudentMessaging: 'Nachrichten zwischen Schüler:innen',
      studentToStudentMessagingHint: 'Erlaubt Schüler:innen, sich gegenseitig Nachrichten zu senden.',
      family: 'Familienverbund',
      soleCustodyEnabled: 'Alleinsorgeberechtigung aktivieren',
      soleCustodyEnabledHint: 'Erlaubt Familien, Alleinsorgeberechtigung zu beantragen (halbiert Pflichtstunden).',
      requireFamilySwitchApproval: 'Familienwechsel-Genehmigung',
      requireFamilySwitchApprovalHint: 'Erfordert Admin-Genehmigung beim Wechsel in eine andere Familie.',
      ldap: {
        title: 'LDAP / Active Directory',
        enabled: 'LDAP-Anmeldung aktivieren',
        enabledHint: 'Benutzer koennen sich mit LDAP/AD-Zugangsdaten anmelden. Lokale Anmeldung bleibt als Fallback aktiv.',
        url: 'Server-URL',
        urlPlaceholder: 'ldap://ldap.example.com:389',
        urlHint: 'LDAP- oder LDAPS-URL (z.B. ldap://server:389 oder ldaps://server:636)',
        baseDn: 'Base DN',
        baseDnPlaceholder: 'dc=example,dc=com',
        baseDnHint: 'Basis-DN fuer die Benutzersuche',
        bindDn: 'Bind DN',
        bindDnPlaceholder: 'cn=admin,dc=example,dc=com',
        bindDnHint: 'DN des Service-Accounts fuer die LDAP-Suche (leer fuer anonymen Bind)',
        bindPassword: 'Bind-Passwort',
        bindPasswordPlaceholder: 'Service-Account-Passwort',
        bindPasswordHint: 'Passwort des Service-Accounts (wird verschluesselt gespeichert)',
        userSearchFilter: 'Benutzer-Suchfilter',
        userSearchFilterHint: "{'{'}0{'}'} wird durch den Benutzernamen ersetzt",
        useSsl: 'SSL verwenden',
        useSslHint: 'SSL/TLS fuer die LDAP-Verbindung aktivieren',
        attrMapping: 'Attribut-Zuordnung',
        attrEmail: 'E-Mail-Attribut',
        attrFirstName: 'Vorname-Attribut',
        attrLastName: 'Nachname-Attribut',
        defaultRole: 'Standard-Rolle',
        defaultRoleHint: 'Rolle fuer automatisch erstellte LDAP-Benutzer',
        testConnection: 'Verbindung testen',
        testSuccess: 'LDAP-Verbindung erfolgreich',
        testFailed: 'LDAP-Verbindung fehlgeschlagen',
        testing: 'Teste Verbindung...',
        saved: 'LDAP-Einstellungen gespeichert',
        passwordNotShown: 'Gespeichert (wird nicht angezeigt)',
      },
    },
    analytics: {
      title: 'Statistiken',
      subtitle: 'Nutzungsübersicht',
      totalUsers: 'Nutzer gesamt',
      activeUsers: 'Aktive Nutzer',
      rooms: 'Räume',
      posts: 'Beiträge',
      events: 'Termine',
      messages: 'Nachrichten',
      postsThisMonth: 'Beiträge diesen Monat',
      newThisWeek: 'Neu diese Woche',
    },
    moduleSubtitle: 'Module aktivieren oder deaktivieren',
    moduleSaved: 'Modul-Konfiguration gespeichert',
    moduleDescriptions: {
      messaging: 'Private Direktnachrichten und Gruppen-Chats zwischen Lehrkräften, Eltern und Schülerinnen',
      files: 'Dateiablage pro Raum: Dokumente, Bilder und Materialien mit Sichtbarkeitssteuerung',
      jobboard: 'Jobbörse für Elternstunden – Angebote, Bewerbungen, Stundenkonto und Jahresabrechnung',
      cleaning: 'Putz-Organisation: Aktionen erstellen, Slots belegen, QR-Check-in beim Putztag',
      calendar: 'Kalender für Raum-, Bereichs- und Schultermine mit RSVP-Funktion',
      forms: 'Formulare und Umfragen erstellen, veröffentlichen und Ergebnisse auswerten (CSV/PDF-Export)',
      fotobox: 'Foto-Galerien in Räumen – Threads anlegen, Bilder hochladen, nach Zielgruppe filtern',
      fundgrube: 'Schulweite Fundgrube: Verlorene Gegenstände mit Foto melden, Besitzer können sie beanspruchen',
      bookmarks: 'Beiträge, Termine, Jobs und Wiki-Seiten als Lesezeichen speichern',
      tasks: 'Kanban-Board pro Raum: Aufgaben erstellen, zuweisen und in konfigurierbaren Spalten verwalten',
      wiki: 'Wiki pro Raum: Markdown-Seiten mit Hierarchie, Versionierung und Suchfunktion',
      profilefields: 'Benutzerdefinierte Profilfelder: Administratoren können zusätzliche Felder für Benutzerprofile anlegen',
      jitsi: 'Jitsi-Videokonferenzen: Video-Meetings in Kalender-Terminen und Raum-Chats starten',
      wopi: 'ONLYOFFICE-Integration: Dokumente direkt im Browser bearbeiten (Word, Excel, PowerPoint)',
      clamav: 'ClamAV-Virenscanner: Hochgeladene Dateien automatisch auf Viren pruefen',
      maintenance: 'Wartungsmodus: System fuer nicht-Administratoren sperren',
      ldap: 'LDAP/Active Directory: Benutzer-Authentifizierung ueber externen Verzeichnisdienst',
      directoryAdminOnly: 'Verzeichnis nur fuer Admins: Benutzerverzeichnis nur fuer Administratoren sichtbar',
      parentletter: 'Elternbriefe: Digitale Elternbriefe mit Bestätigungsrücklauf, Variablen und automatischem Mahnwesen',
    },
    theme: {
      groupPrimary: 'Primär & Akzente',
      groupBackground: 'Hintergründe',
      groupText: 'Text',
      groupBorder: 'Rahmen',
      groupStatus: 'Statusfarben',
      colorSchemeHint: 'Jede Farbe wirkt sich auf alle Elemente dieses Typs aus (eigene Komponenten und PrimeVue). Tipp: Gelb-/helle Flächen brauchen dunklen Text.',
      reset: 'Auf Standard zurücksetzen',
      primaryColor: 'Primärfarbe',
      primaryHover: 'Primärfarbe (Hover)',
      primaryDark: 'Primär dunkel (Rahmen/Fokus)',
      primaryContrast: 'Text auf Primärfarbe',
      secondaryColor: 'Sekundärfarbe',
      linkColor: 'Linkfarbe',
      focusRing: 'Fokus-Ring',
      background: 'Hintergrund',
      cardBg: 'Karten-Hintergrund',
      sidebarBg: 'Sidebar-Hintergrund',
      activeBg: 'Aktiver Hintergrund',
      highlightBg: 'Hervorhebung (Auswahl)',
      textColor: 'Textfarbe',
      secondaryText: 'Sekundäre Textfarbe',
      mutedText: 'Gedämpfte Textfarbe',
      borderColorStrong: 'Rahmenfarbe (kräftig)',
      borderColor: 'Rahmenfarbe (hell)',
      successColor: 'Erfolg',
      warningColor: 'Warnung',
      dangerColor: 'Fehler/Gefahr',
      infoColor: 'Info',
      uploadLogo: 'Logo hochladen',
      schoolNameFallback: 'Schulname',
      previewText: 'Dies ist eine Vorschau des Farbschemas. Die Farben werden nach dem Speichern angewendet.',
      primaryButton: 'Primär-Button',
      secondaryButton: 'Sekundär-Button',
    },
    reportSubtitle: 'Stundenübersicht aller Familien (Elternstunden + Putzstunden)',
    columnName: 'Name',
    columnEmail: 'E-Mail',
    columnRole: 'Rolle',
    columnSlug: 'Slug',
    columnOrder: 'Reihenfolge',
    columnType: 'Typ',
    columnMembers: 'Mitglieder',
    sectionDialog: 'Schulbereich anlegen',
    sectionsAndRooms: 'Schulbereiche & Räume',
    editSection: 'Bereich bearbeiten',
    editSectionDialog: 'Schulbereich bearbeiten',
    deleteSection: 'Bereich löschen',
    deleteSectionConfirm: 'Bereich \u00AB{name}\u00BB wirklich deaktivieren?',
    deleteSectionWarn: 'Zugeordnete Räume verlieren ihre Bereichszuordnung.',
    roomCount: '{count} Raum | {count} Räume',
    noSectionRooms: 'Ohne Bereich',
    sectionSaved: 'Bereich gespeichert',
    columnSection: 'Bereich',
    editRoom: 'Raum bearbeiten',
    deactivateRoom: 'Raum deaktivieren',
    reactivateRoom: 'Raum reaktivieren',
    deleteRoom: 'Raum löschen',
    deactivateRoomConfirm: 'Raum \u00AB{name}\u00BB wirklich deaktivieren? Er wird für Nutzer nicht mehr sichtbar sein.',
    reactivateRoomConfirm: 'Raum \u00AB{name}\u00BB wirklich reaktivieren?',
    deleteRoomConfirm: 'Raum \u00AB{name}\u00BB wirklich endgültig löschen?',
    deleteRoomWarn: 'Alle Mitgliedschaften, Diskussionen, Dateien und Chat-Nachrichten werden unwiderruflich gelöscht.',
    roomSaved: 'Raum gespeichert',
    roomDeactivated: 'Raum deaktiviert',
    roomReactivated: 'Raum reaktiviert',
    roomDeleted: 'Raum gelöscht',
    deactivated: 'Deaktiviert',
    editUser: 'Benutzer bearbeiten',
    userSaved: 'Benutzer gespeichert',
    pendingUsers: 'Neue Benutzer',
    approve: 'Freischalten',
    userApproved: 'Benutzer freigeschaltet',
    tabProfile: 'Profil',
    tabRooms: 'Räume',
    tabFamily: 'Familie',
    tabDsgvo: 'DSGVO',
    dsgvoExportData: 'Daten exportieren',
    dsgvoExportDesc: 'Alle personenbezogenen Daten dieses Benutzers als JSON exportieren.',
    dsgvoExportSuccess: 'Datenexport heruntergeladen',
    dsgvoRequestDeletion: 'Löschung anfordern',
    dsgvoRequestDeletionDesc: 'Das Konto wird nach einer 14-tägigen Frist endgültig gelöscht.',
    dsgvoDeletionRequested: 'Löschung angefordert',
    dsgvoDeletionScheduled: 'Konto wird am {date} gelöscht.',
    dsgvoCancelDeletion: 'Löschung abbrechen',
    dsgvoDeletionCancelled: 'Löschung abgebrochen',
    dsgvoNoDeletion: 'Keine Löschung angefordert.',
    dsgvoConfirmTitle: 'Benutzerkonto löschen?',
    dsgvoConfirmMessage: 'Das Konto von {name} wird in 14 Tagen endgültig gelöscht. Alle personenbezogenen Daten werden unwiderruflich entfernt.',
    searchRoom: 'Raum suchen...',
    searchUser: 'Benutzer suchen...',
    addMember: 'Hinzufügen',
    memberAdded: 'Mitglied hinzugefügt',
    memberRemoved: 'Mitglied entfernt',
    noRoomMemberships: 'Keine Raum-Mitgliedschaften',
    noFamilyMemberships: 'Keine Familien-Mitgliedschaften',
    selectFamily: 'Familie auswählen',
    addToFamily: 'Hinzufügen',
    familyMemberAdded: 'Familienmitglied hinzugefügt',
    familyMemberRemoved: 'Familienmitglied entfernt',
    hoursExempt: 'Befreit',
    exemptHours: 'Von Elternstunden befreien',
    requireHours: 'Elternstunden-Pflicht aktivieren',
    toggleHoursExempt: 'Stundenpflicht umschalten',
    familyExempted: 'Familie von Elternstunden befreit',
    familyNotExempted: 'Elternstunden-Pflicht aktiviert',
    exemptFamilies: 'Befreite Familien',
    exemptFamiliesHint: 'Diese Familien sind von der Elternstunden-Pflicht befreit.',
    manageMembers: 'Mitglieder verwalten',
    moveMember: 'Verschieben',
    copyMember: 'Kopieren',
    memberMoved: 'Mitglied verschoben',
    memberCopied: 'Mitglied kopiert',
    targetRoom: 'Ziel-Raum',
    selectRoom: 'Raum auswählen',
    allSections: 'Alle Bereiche',
    allTypes: 'Alle Typen',
    sectionAdmins: 'Bereichsleitung',
    sectionAdminAdded: 'Bereichsleitung hinzugefügt',
    sectionAdminRemoved: 'Bereichsleitung entfernt',
    noSectionAdmins: 'Keine Bereichsleitung zugewiesen',
    roomSectionChanged: 'Bereichszuordnung geändert',
    filterByRole: 'Nach Rolle filtern',
    allRoles: 'Alle Rollen',
    filterBySpecialRole: 'Nach Sonderrolle filtern',
    allSpecialRoles: 'Alle Sonderrollen',
    specialRoleLabels: {
      PUTZORGA: 'Putz-Orga',
      ELTERNBEIRAT: 'Elternbeirat',
    },
    sectionAdminSections: 'Zuständige Bereiche',
    selectSections: 'Bereiche auswählen...',
    filterByStatus: 'Nach Status filtern',
    allStatuses: 'Alle',
    searchUsers: 'Name oder E-Mail suchen...',
    noUsersFound: 'Keine Benutzer gefunden',
    assignedRoles: 'Zugewiesene Rollen',
    assignedRolesHint: 'Rollen, zwischen denen der Benutzer wechseln kann',
    specialRoles: 'Sonderrollen',
    allSectionsGlobal: 'Alle Bereiche (global)',
    families: 'Familien',
    familiesTitle: 'Familienverwaltung',
    familiesSubtitle: 'Alle Familienverbunde anzeigen und verwalten',
    searchFamilies: 'Familie suchen...',
    editFamily: 'Familie bearbeiten',
    deleteFamily: 'Familie löschen',
    deleteFamilyConfirm: 'Familie \u00AB{name}\u00BB wirklich löschen? Alle Mitgliedschaften werden entfernt.',
    familyName: 'Familienname',
    familyUpdated: 'Familie aktualisiert',
    familyDeleted: 'Familie gelöscht',
    familyDeactivated: 'Familie deaktiviert',
    familyActivated: 'Familie aktiviert',
    familyActive: 'Aktiv',
    deactivate: 'Deaktivieren',
    activate: 'Aktivieren',
    memberRole: 'Rolle',
    searchUserPlaceholder: 'Name oder E-Mail eingeben...',
    tabInfo: 'Info',
    tabMembers: 'Mitglieder',
    tabHours: 'Stunden',
    parents: 'Eltern',
    children: 'Kinder',
    columnHours: 'Stunden',
    noFamilies: 'Keine Familien vorhanden',
    noFamiliesFound: 'Keine Familien gefunden',
    removeMember: 'Mitglied entfernen',
    removeMemberConfirm: '{name} wirklich aus der Familie entfernen?',
    jobHours: 'Elternstunden',
    cleaningHours: 'Putzstunden',
    totalHours: 'Gesamt',
    targetHours: 'Soll',
    balance: 'Saldo',
  },
  jobboard: {
    title: 'Jobbörse',
    create: 'Job erstellen',
    openJobs: 'Offene Jobs',
    myAssignments: 'Meine Aufgaben',
    noJobs: 'Keine offenen Jobs vorhanden.',
    noAssignments: 'Sie haben keine aktiven Aufgaben.',
    apply: 'Anmelden',
    assignees: 'Helfer',
    report: 'Familien-Stundenbericht',
    category: 'Kategorie',
    location: 'Ort',
    estimatedHours: 'Geschätzte Stunden',
    date: 'Datum',
    time: 'Uhrzeit',
    contact: 'Kontakt',
    maxHelpers: 'Max. Helfer',
    allCategories: 'Alle Kategorien',
    categoryNormal: 'Normal',
    categoryReinigung: 'Reinigung',
    filterCategory: 'Kategorie filtern',
    hoursWorked: 'geleistet',
    confirmed: 'Bestätigt',
    pendingConfirmation: 'Warte auf Bestätigung',
    assignments: 'Zuweisungen',
    noAssignmentsYet: 'Noch keine Zuweisungen',
    pendingTab: 'Zu bestätigen',
    noPendingConfirmations: 'Keine offenen Bestätigungen.',
    reject: 'Ablehnen',
    hoursRejected: 'Stunden abgelehnt',
    cleaningAssignments: 'Putzaktionen',
    jobAssignments: 'Job-Aufgaben',
    confirmDuration: 'Dauer bestätigen',
    confirmDurationTitle: 'Putz-Dauer bestätigen',
    confirmDurationDesc: 'Bitte überprüfen und bestätigen Sie die tatsächliche Dauer Ihres Putzeinsatzes.',
    actualMinutes: 'Tatsächliche Dauer',
    durationConfirmed: 'Dauer bestätigt',
    minutes: 'Minuten',
    completedJobs: 'Abgeschlossene Jobs',
    noCompletedJobs: 'Keine abgeschlossenen Jobs vorhanden.',
    createPrivate: 'Eigener Job',
    draftsTab: 'Entwürfe',
    noDraftJobs: 'Keine Entwürfe vorhanden.',
    approveJob: 'Freigeben',
    jobApproved: 'Job freigegeben',
    sortByDate: 'Nach Datum',
    sortByName: 'Nach Name',
    visibility: {
      PRIVATE: 'Privat',
      DRAFT: 'Entwurf',
      PUBLIC: 'Öffentlich',
    },
    returnJob: 'Zurückgeben',
    returnJobTitle: 'Job zurückgeben',
    returnJobConfirm: 'Möchten Sie diesen Job wirklich zurückgeben? Er wird wieder in der Jobbörse verfügbar.',
    complete: 'Abschließen',
    completeTask: 'Aufgabe abschließen',
    actualHours: 'Tatsächliche Stunden',
    notesOptional: 'Anmerkungen (optional)',
    create_form: {
      titlePlaceholder: 'z.B. Schulhof kehren',
      categoryPlaceholder: 'z.B. Garten, Reparatur, Veranstaltung',
      locationPlaceholder: 'z.B. Schulhof, Werkraum',
      timePlaceholder: 'z.B. 14:00 - 16:00',
      contactPlaceholder: 'z.B. Telefon, E-Mail',
    },
    titleLabel: 'Titel',
    linkedEvent: 'Verknüpfte Veranstaltung',
    noLinkedEvent: 'Keine Verknüpfung',
    selectEvent: 'Veranstaltung auswählen',
    filterByEvent: 'Nach Veranstaltung filtern',
    allEvents: 'Alle Veranstaltungen',
    linkedJobs: 'Verknüpfte Jobs',
    noLinkedJobs: 'Keine verknüpften Jobs vorhanden.',
    createLinkedJob: 'Job erstellen',
    linkExistingJob: 'Bestehenden Job verknüpfen',
    selectJobToLink: 'Job zum Verknüpfen auswählen',
    linkJob: 'Verknüpfen',
    jobLinked: 'Job erfolgreich verknüpft',
    noOpenJobs: 'Keine offenen Jobs zum Verknüpfen verfügbar.',
    deleteConfirm: 'Möchten Sie diesen Job wirklich endgültig löschen?',
    editJob: 'Job bearbeiten',
    deleteConfirmTitle: 'Job löschen',
    deleted: 'Job gelöscht',
    jobCount: '{n} Job(s)',
    hoursConfirmed: 'Stunden bestätigt',
    hoursConfirmedDetail: '{hours}h wurden dem Stundenkonto der Familie {family} gutgeschrieben.',
    filterByDate: 'Nach Datum filtern',
    fromDate: 'Von',
    toDate: 'Bis',
    clearDateFilter: 'Datumsfilter zurücksetzen',
    statuses: {
      OPEN: 'Offen',
      PARTIALLY_ASSIGNED: 'Teilweise vergeben',
      ASSIGNED: 'Vergeben',
      IN_PROGRESS: 'In Arbeit',
      COMPLETED: 'Abgeschlossen',
      CANCELLED: 'Abgesagt',
    },
    assignmentStatuses: {
      ASSIGNED: 'Zugewiesen',
      IN_PROGRESS: 'In Arbeit',
      COMPLETED: 'Erledigt',
      CANCELLED: 'Abgesagt',
    },
    room: 'Raum',
    selectRoom: 'Raum auswählen',
    noRoom: 'Kein Raum',
    attachments: 'Anhänge',
    noAttachments: 'Keine Anhänge vorhanden.',
    uploadAttachment: 'Datei hochladen',
    attachmentUploaded: 'Datei hochgeladen',
    maxAttachments: 'Max. 5 Dateien, je max. 10 MB',
    fileTooLarge: 'Datei zu groß. Maximal 10 MB.',
  },
  feed: {
    title: 'Feed',
    post: 'Veröffentlichen',
    postCreated: 'Beitrag veröffentlicht',
    comment: 'Kommentieren',
    pinned: 'Angeheftet',
    pin: 'Anheften',
    unpin: 'Loslösen',
    noPosts: 'Noch keine Beiträge vorhanden.',
    loadMore: 'Weitere laden',
    deleteConfirm: 'Beitrag wirklich löschen?',
    confirmDeleteTitle: 'Beitrag löschen',
    confirmDeleteMessage: 'Möchten Sie diesen Beitrag wirklich löschen? Dies kann nicht rückgängig gemacht werden.',
    titlePlaceholder: 'Titel (optional)',
    contentPlaceholder: 'Was gibt es Neues?',
    commentPlaceholder: 'Kommentar schreiben...',
    linkPreview: 'Link-Vorschau',
    videoEmbed: 'Video',
    attachFiles: 'Dateien anhängen',
    attachments: 'Anhänge',
    removeFile: 'Datei entfernen',
    filesSelected: '{count} Datei(en) ausgewählt',
    fileTooLarge: 'Datei zu groß (max. 50 MB)',
    tooManyFiles: 'Maximal 10 Dateien erlaubt',
    uploadingAttachments: 'Anhänge werden hochgeladen...',
    sourceLabels: {
      ROOM: 'Raum',
      SECTION: 'Schulbereich',
      SCHOOL: 'Schulweit',
      BOARD: 'Pinnwand',
      SYSTEM: 'System',
    },
  },
  notifications: {
    title: 'Benachrichtigungen',
    markAllRead: 'Alle gelesen',
    noNotifications: 'Keine Benachrichtigungen',
    justNow: 'Gerade eben',
    minutesAgo: 'vor {n}m',
    hoursAgo: 'vor {n}h',
    daysAgo: 'vor {n}d',
    bellWithCount: '{count} ungelesene Benachrichtigungen',
  },
  files: {
    title: 'Dateien',
    upload: 'Hochladen',
    newFolder: 'Neuer Ordner',
    noFiles: 'Noch keine Dateien vorhanden',
    folderName: 'Ordnername',
    audience: 'Sichtbarkeit',
    audienceAll: 'Alle',
    audienceParents: 'Nur Eltern',
    audienceStudents: 'Nur Sch\u00fcler',
  },
  wopi: {
    title: 'ONLYOFFICE / Dokumentbearbeitung',
    enabled: 'ONLYOFFICE-Integration aktivieren',
    hint: 'ONLYOFFICE Document Server muss als separater Container laufen. Aktivieren Sie das "office" Profil in Docker Compose.',
    officeUrl: 'ONLYOFFICE Server-URL',
    officeUrlHint: 'Die URL des ONLYOFFICE Document Servers (z.B. https://office.example.com:8443)',
    editOnline: 'Online bearbeiten',
    backToFiles: 'Zur\u00fcck zu Dateien',
    editorTitle: 'Dokumenteditor',
    errorLoading: 'Fehler beim Laden des Editors',
    saved: 'ONLYOFFICE-Einstellungen gespeichert',
  },
  messages: {
    title: 'Nachrichten',
    newConversation: 'Neue Konversation',
    newMessage: 'Neue Nachricht',
    noConversations: 'Noch keine Konversationen',
    selectConversation: 'Wählen Sie eine Konversation',
    writePlaceholder: 'Nachricht schreiben...',
    conversation: 'Konversation',
    searchUser: 'Empfänger suchen',
    searchUserPlaceholder: 'Name oder E-Mail eingeben...',
    startConversation: 'Konversation starten',
    groupConversation: 'Gruppennachricht',
    groupName: 'Gruppenname',
    groupNamePlaceholder: 'Name der Gruppe...',
    communicationNotAllowed: 'Kommunikation zwischen diesen Nutzern ist nicht erlaubt',
    deleteTitle: 'Konversation löschen',
    deleteConfirm: 'Möchten Sie diese Konversation wirklich löschen? Sie wird aus Ihrer Liste entfernt.',
    send: 'Nachricht senden',
    image: 'Bild',
    attachImage: 'Bild anhängen',
    replyTo: 'Antworten',
    imageUploadError: 'Fehler beim Hochladen des Bildes',
    invalidImageType: 'Ungültiges Bildformat. Erlaubt: JPEG, PNG, WebP, GIF',
    fileTooLarge: 'Datei ist zu groß. Maximal 10 MB erlaubt.',
    mute: 'Stummschalten',
    unmute: 'Stummschaltung aufheben',
    muted: 'Stumm',
    unmuted: 'Stummschaltung aufgehoben',
    file: 'Datei',
    attachFile: 'PDF anhängen',
    linkFile: 'Datei verlinken',
    selectRoom: 'Raum auswählen',
    noFiles: 'Keine Dateien in diesem Raum',
  },
  cleaning: {
    title: 'Putz-Orga',
    manage: 'Verwaltung',
    upcomingSlots: 'Anstehende Termine',
    mySlots: 'Meine Termine',
    noSlots: 'Keine anstehenden Putztermine.',
    noMySlots: 'Sie haben sich für keine Termine eingetragen.',
    pendingTab: 'Zu bestätigen',
    noPendingConfirmations: 'Keine offenen Bestätigungen.',
    hoursConfirmed: 'Putzstunden bestätigt',
    hoursRejected: 'Putzstunden abgelehnt',
    date: 'Datum',
    time: 'Uhrzeit',
    participants: 'Teilnehmer',
    minRequired: 'Mind. {n} benötigt',
    slotStatus: 'Belegung',
    enoughParticipants: 'Genug Helfer',
    needMore: 'Noch Helfer nötig',
    register: 'Eintragen',
    unregister: 'Austragen',
    registered: 'Erfolgreich eingetragen',
    unregistered: 'Eintrag zurückgezogen',
    checkIn: 'Einchecken',
    checkOut: 'Auschecken',
    checkedIn: 'Erfolgreich eingecheckt',
    checkedOut: 'Erfolgreich ausgecheckt',
    offerSwap: 'Tausch anbieten',
    swapOffered: 'Tausch angeboten',
    swapAvailable: 'Tausch',
    noShow: 'Nicht erschienen',
    registrations: 'Anmeldungen',
    checkedInCol: 'Eingecheckt',
    checkedOutCol: 'Ausgecheckt',
    duration: 'Dauer',
    swapCol: 'Status',
    linkedJob: 'Dieser Putztermin hat einen verknüpften Job in der Jobbörse.',
    viewJob: 'Zur Jobbörse',
    qrCheckIn: 'QR-Check-in',
    qrInstructions: 'Bitte scannen Sie den QR-Code am Putzort oder geben Sie den Code manuell ein.',
    qrPlaceholder: 'QR-Token eingeben...',
    status: {
      OPEN: 'Offen',
      FULL: 'Voll',
      IN_PROGRESS: 'Laufend',
      COMPLETED: 'Abgeschlossen',
      CANCELLED: 'Abgesagt',
    },
    days: {
      monday: 'Montag',
      tuesday: 'Dienstag',
      wednesday: 'Mittwoch',
      thursday: 'Donnerstag',
      friday: 'Freitag',
    },
    createForRoom: 'Putzaktion erstellen',
    admin: {
      title: 'Putz-Organisation',
      newConfig: 'Neue Putzaktion',
      configTitle: 'Titel',
      scope: 'Bereich oder Raum',
      section: 'Schulbereich',
      room: 'Raum',
      selectRoom: 'Raum auswählen',
      searchRoom: 'Raum suchen...',
      sectionId: 'Schulbereich',
      sectionIdPlaceholder: 'Schulbereich auswählen',
      day: 'Wochentag',
      timeRange: 'Zeitraum',
      startTime: 'Beginn',
      endTime: 'Ende',
      participants: 'Teilnehmer',
      minParticipants: 'Mind. Teilnehmer',
      maxParticipants: 'Max. Teilnehmer',
      hoursCredit: 'Stundengutschrift',
      status: 'Status',
      generate: 'Termine generieren',
      generateTitle: 'Putztermine generieren',
      generateHint: 'Termine für "{title}" im gewählten Zeitraum erstellen.',
      fromDate: 'Von',
      toDate: 'Bis',
      configCreated: 'Putzaktion erstellt — Termine wurden automatisch generiert',
      specificDate: 'Konkretes Datum',
      specificDateHint: 'Optional: Für einmalige Putzaktionen',
      orRecurring: 'Oder wiederkehrend (Wochentag):',
      slotsGenerated: '{n} Termine generiert',
      exportQrCodes: 'QR-Codes PDF',
      exportQrCodesTitle: 'QR-Codes als PDF exportieren',
      exportQrCodesHint: 'QR-Code-Seite für "{title}" im gewählten Zeitraum erstellen.',
      putzOrgaManagement: 'PutzOrga-Verwaltung',
      putzOrgaHint: 'Weisen Sie Eltern die PutzOrga-Rolle für einen Schulbereich zu. PutzOrgas können Putztermine verwalten.',
      selectSection: 'Schulbereich auswählen',
      assignPutzOrga: 'PutzOrga zuweisen',
      searchParent: 'Elternteil suchen...',
      assign: 'Zuweisen',
      noPutzOrga: 'Keine PutzOrga für diesen Bereich zugewiesen.',
      putzOrgaAssigned: 'PutzOrga-Rolle zugewiesen',
      putzOrgaRemoved: 'PutzOrga-Rolle entfernt',
      registrations: 'Anmeldungen',
      showRegistrations: 'Anmeldungen anzeigen',
      registrationsTitle: 'Anmeldungen: {title}',
      noRegistrations: 'Keine Anmeldungen vorhanden.',
      family: 'Familie',
      assignmentStatus: 'Status',
      hoursCol: 'Stunden',
      participantCircle: 'Teilnehmerkreis einschränken',
      participantCircleNone: 'Alle (Schulbereich)',
      participantCircleRoom: 'Nur Raum-Mitglieder',
      participantCircleId: 'Raum auswählen',
    },
  },
  discover: {
    title: 'Räume entdecken',
    searchPlaceholder: 'Nach Räumen suchen...',
    createRoom: 'Raum erstellen',
    join: 'Beitreten',
    joined: 'Erfolgreich beigetreten',
    created: 'Raum erstellt',
    members: 'Mitglieder',
    noRooms: 'Keine Räume gefunden.',
    tags: 'Tags',
    tagsPlaceholder: 'Tag eingeben und Enter drücken',
    allSections: 'Alle Bereiche',
    allTypes: 'Alle Typen',
    otherRooms: 'Sonstige Räume',
    filterBySection: 'Bereich',
    filterByType: 'Typ',
  },
  discussions: {
    title: 'Diskussionen',
    create: 'Neue Diskussion',
    createTitle: 'Diskussion erstellen',
    noThreads: 'Noch keine Diskussionen vorhanden',
    titleLabel: 'Titel',
    titlePlaceholder: 'Thema der Diskussion...',
    contentLabel: 'Beschreibung (optional)',
    contentPlaceholder: 'Worum geht es in dieser Diskussion?',
    replies: '{n} Antworten',
    noReplies: 'Noch keine Antworten',
    replyPlaceholder: 'Antwort schreiben...',
    archived: 'Archiviert',
    archive: 'Archivieren',
    archivedNotice: 'Diese Diskussion ist archiviert. Antworten sind nicht mehr möglich.',
    audience: 'Zielgruppe',
    audienceAlle: 'Alle',
    audienceEltern: 'Eltern',
    audienceKinder: 'Kinder',
    loadError: 'Diskussionen konnten nicht geladen werden',
  },
  calendar: {
    title: 'Kalender',
    viewList: 'Liste',
    viewMonth: '1 Monat',
    view3Months: '3 Monate',
    viewYear: 'Schuljahr',
    createEvent: 'Termin erstellen',
    editEvent: 'Termin bearbeiten',
    noEvents: 'Keine Termine in diesem Zeitraum.',
    eventTitle: 'Titel',
    titlePlaceholder: 'z.B. Elternabend, Schulfest',
    description: 'Beschreibung',
    descriptionPlaceholder: 'Weitere Details zum Termin...',
    location: 'Ort',
    locationPlaceholder: 'z.B. Aula, Klassenzimmer 3',
    allDay: 'Ganztägig',
    startDate: 'Startdatum',
    startTime: 'Startzeit',
    endDate: 'Enddatum',
    endTime: 'Endzeit',
    scope: 'Sichtbarkeit',
    scopes: {
      ROOM: 'Raum',
      SECTION: 'Schulbereich',
      SCHOOL: 'Schulweit',
    },
    selectRoom: 'Raum auswählen',
    selectSection: 'Schulbereich auswählen',
    recurrence: 'Wiederholung',
    recurrences: {
      NONE: 'Keine',
      DAILY: 'Täglich',
      WEEKLY: 'Wöchentlich',
      MONTHLY: 'Monatlich',
      YEARLY: 'Jährlich',
    },
    recurrenceEnd: 'Wiederholung bis',
    rsvp: 'Teilnahme',
    attending: 'Zusage',
    maybe: 'Vielleicht',
    declined: 'Absage',
    attendingCount: '{n} Zusagen',
    maybeCount: '{n} Vielleicht',
    declinedCount: '{n} Absagen',
    cancelled: 'Abgesagt',
    cancelEvent: 'Termin absagen',
    cancelConfirm: 'Termin wirklich absagen? Alle Teilnehmer werden benachrichtigt.',
    deleteConfirm: 'Termin wirklich löschen?',
    created: 'Termin erstellt',
    updated: 'Termin aktualisiert',
    today: 'Heute',
    month: 'Monat',
    threeMonth: '3 Monate',
    year: 'Jahr',
    agenda: 'Agenda',
    showCleaning: 'Putzaktionen anzeigen',
    cleaning: 'Putzaktion',
    exportCalendar: 'Exportieren',
    exportEvent: 'Als .ics exportieren',
    color: 'Farbe',
    colorHint: 'Wähle eine Farbe für diesen Termin im Kalender',
    customColor: 'Eigene Farbe',
    ical: {
      title: 'iCal-Abonnements',
      addSubscription: 'Abonnement hinzufügen',
      name: 'Name',
      url: 'URL',
      lastSynced: 'Letzte Synchronisierung',
      sync: 'Jetzt synchronisieren',
      synced: 'Synchronisierung gestartet',
      deleted: 'Abonnement gelöscht',
      created: 'Abonnement erstellt',
      noSubscriptions: 'Keine iCal-Abonnements vorhanden.',
      deleteConfirm: 'Abonnement wirklich löschen?',
      namePlaceholder: 'z.B. Schulferien Bayern',
      urlPlaceholder: 'https://example.com/calendar.ics',
      color: 'Farbe',
      showImported: 'Importierte Termine anzeigen',
    },
    filterPlaceholder: 'Bereiche & Räume filtern',
    scopeSchool: 'Schulweit',
    scopeSections: 'Bereiche',
    scopeRooms: 'Räume',
    showJobs: 'Offene Jobs anzeigen',
    vacation: 'Schulferien',
    holiday: 'Feiertag',
    rsvpStatus: {
      ATTENDING: 'Zugesagt',
      MAYBE: 'Vielleicht',
      DECLINED: 'Abgesagt',
    },
    jobEntry: 'Job',
    noEventsForDay: 'Keine Termine an diesem Tag.',
  },
  forms: {
    title: 'Formulare & Umfragen',
    create: 'Formular erstellen',
    createForm: 'Formular erstellen',
    editForm: 'Formular bearbeiten',
    noForms: 'Keine Formulare verfügbar.',
    noMyForms: 'Sie haben noch keine Formulare erstellt.',
    formTitle: 'Titel',
    titlePlaceholder: 'z.B. Elternumfrage, Einverständniserklärung',
    description: 'Beschreibung',
    formType: 'Typ',
    scope: 'Sichtbarkeit',
    selectRoom: 'Raum auswählen',
    selectSections: 'Schulbereiche auswählen',
    pendingForms: 'Offene Formulare',
    viewAll: 'Alle anzeigen',
    deadline: 'Frist',
    deadlineLabel: 'Frist',
    anonymousLabel: 'Anonym',
    anonymous: 'Anonym',
    responded: 'Beantwortet',
    questionsCount: 'Fragen',
    responsesCount: 'Antworten',
    tabs: {
      available: 'Verfügbar',
      mine: 'Meine',
    },
    types: {
      SURVEY: 'Umfrage',
      CONSENT: 'Einverständnis',
    },
    scopes: {
      ROOM: 'Raum',
      SECTION: 'Schulbereich',
      SCHOOL: 'Schulweit',
    },
    statuses: {
      DRAFT: 'Entwurf',
      PUBLISHED: 'Veröffentlicht',
      CLOSED: 'Geschlossen',
      ARCHIVED: 'Archiviert',
    },
    questionTypes: {
      TEXT: 'Freitext',
      SINGLE_CHOICE: 'Einfachauswahl',
      MULTIPLE_CHOICE: 'Mehrfachauswahl',
      RATING: 'Bewertung',
      YES_NO: 'Ja/Nein',
    },
    questions: 'Fragen',
    addQuestion: 'Frage hinzufügen',
    removeQuestion: 'Frage entfernen',
    questionType: 'Fragetyp',
    questionLabel: 'Fragetext',
    questionDescription: 'Beschreibung (optional)',
    required: 'Pflichtfeld',
    options: 'Antwortoptionen',
    option: 'Option',
    addOption: 'Option hinzufügen',
    noQuestions: 'Noch keine Fragen hinzugefügt. Klicken Sie auf "Frage hinzufügen".',
    saveDraft: 'Als Entwurf speichern',
    publish: 'Veröffentlichen',
    close: 'Schließen',
    saved: 'Formular gespeichert',
    published: 'Formular veröffentlicht',
    closed: 'Formular geschlossen',
    deleted: 'Formular gelöscht',
    editQuestions: 'Fragen bearbeiten',
    deleteForm: 'Formular löschen',
    viewResults: 'Ergebnisse anzeigen',
    submitResponse: 'Antwort absenden',
    alreadyResponded: 'Sie haben dieses Formular bereits beantwortet.',
    thankYou: 'Vielen Dank für Ihre Antwort!',
    yes: 'Ja',
    no: 'Nein',
    resultsSummary: 'Ergebnisse',
    responseRate: 'Rücklaufquote',
    average: 'Durchschnitt',
    exportCsv: 'CSV Export',
    exportPdf: 'PDF Export',
    individualResponses: 'Einzelantworten',
    user: 'Benutzer',
    submittedAt: 'Eingereicht am',
    editResponse: 'Antwort bearbeiten',
    updateResponse: 'Antwort aktualisieren',
    responseUpdated: 'Antwort aktualisiert',
    archive: 'Archivieren',
    formArchived: 'Formular archiviert',
    viewResultsHint: 'Ergebnisse ansehen',
    deadlinePassed: 'Frist abgelaufen',
    canEditUntil: 'Bearbeitbar bis',
    cannotEditAnonymous: 'Anonyme Antworten können nicht bearbeitet werden',
  },
  chat: {
    title: 'Chat',
    placeholder: 'Nachricht schreiben...',
    noMessages: 'Noch keine Nachrichten in diesem Kanal.',
    mute: 'Stummschalten',
    unmute: 'Stummschaltung aufheben',
    muted: 'Stumm',
    today: 'Heute',
    yesterday: 'Gestern',
    send: 'Nachricht senden',
    image: 'Bild',
    attachImage: 'Bild anhängen',
    replyTo: 'Antworten',
    channels: {
      MAIN: 'Alle',
      PARENTS: 'Eltern-Lehrer Chat',
      STUDENTS: 'Schüler-Lehrer Chat',
    },
  },
  fotobox: {
    title: 'Fotobox',
    threads: 'Bilder-Threads',
    newThread: 'Neuer Thread',
    noThreads: 'Noch keine Bilder-Threads vorhanden.',
    threadTitle: 'Titel',
    threadDescription: 'Beschreibung',
    images: 'Bilder',
    imageCount: '{count} Bild | {count} Bilder',
    uploadImages: 'Bilder hochladen',
    deleteThread: 'Thread löschen',
    deleteImage: 'Bild löschen',
    caption: 'Bildunterschrift',
    settings: 'Fotobox-Einstellungen',
    enabled: 'Fotobox aktiviert',
    permission: 'Standard-Berechtigung',
    permissionViewOnly: 'Nur ansehen',
    permissionPostImages: 'Bilder posten',
    permissionCreateThreads: 'Threads erstellen',
    maxFileSize: 'Max. Dateigröße (MB)',
    maxImagesPerThread: 'Max. Bilder pro Thread',
    unlimited: 'Unbegrenzt',
    confirmDeleteThread: 'Thread und alle Bilder wirklich löschen?',
    confirmDeleteImage: 'Bild wirklich löschen?',
    uploadSuccess: '{count} Bild(er) hochgeladen',
    dragDropHint: 'Bilder hierher ziehen oder klicken zum Auswählen',
    fileTooLarge: 'Datei zu groß (max. {max} MB)',
    invalidFileType: 'Ungültiger Dateityp. Erlaubt: JPEG, PNG, WebP, GIF',
    back: 'Zurück zur Übersicht',
    noImages: 'Noch keine Bilder in diesem Thread.',
    editCaption: 'Bildunterschrift bearbeiten',
    close: 'Schließen',
    previous: 'Vorheriges Bild',
    next: 'Nächstes Bild',
    of: 'von',
    photoConsentRequired: 'Foto-Einwilligung erforderlich. Bitte erteile sie unter den Datenschutz-Einstellungen.',
    privacySettings: 'Datenschutz-Einstellungen',
  },
  sectionAdmin: {
    title: 'Bereichsverwaltung',
    selectSection: 'Schulbereich auswählen',
    users: 'Benutzer im Bereich',
    assignRole: 'Rolle zuweisen',
    removeRole: 'Rolle entziehen',
    createRoom: 'Raum erstellen',
    specialRoles: 'Sonderrollen',
    noUsers: 'Keine Benutzer im Bereich',
    roleAssigned: 'Rolle zugewiesen',
    roleRemoved: 'Rolle entfernt',
    roomCreated: 'Raum erstellt',
    onlyAllowedRoles: 'Erlaubte Rollen: PUTZORGA, ELTERNBEIRAT',
    roomName: 'Raumname',
    roomType: 'Raumtyp',
    roomDescription: 'Beschreibung',
    noSections: 'Keine Schulbereiche zugewiesen',
    tabUsers: 'Benutzer',
    tabRooms: 'Räume',
    allRoles: 'Alle Rollen',
    allTypes: 'Alle Typen',
    searchUsers: 'Benutzer suchen...',
    searchRooms: 'Räume suchen...',
    memberCount: 'Mitglieder',
    noRooms: 'Keine Räume im Bereich',
    viewMembers: 'Mitglieder anzeigen',
  },
  profile: {
    title: 'Mein Profil',
    editProfile: 'Profil bearbeiten',
    saved: 'Profil gespeichert',
    language: 'Sprache',
    roles: 'Meine Rollen',
    roleLabels: {
      SUPERADMIN: 'Superadmin',
      SECTION_ADMIN: 'Bereichsleitung',
      TEACHER: 'Lehrkraft',
      PARENT: 'Elternteil',
      STUDENT: 'Schüler/in',
    },
    activeRole: 'Aktive Rolle',
    switchRole: 'Rolle wechseln',
    roleSwitched: 'Rolle gewechselt zu {role}',
    assignedRoles: 'Zugewiesene Rollen',
    pushNotifications: 'Push-Benachrichtigungen',
    enablePush: 'Push-Benachrichtigungen aktivieren',
    pushDenied: 'Push-Benachrichtigungen wurden im Browser blockiert. Bitte erlauben Sie diese in den Browser-Einstellungen.',
    mutedChats: 'Stummgeschaltete Chats',
    noMutedChats: 'Keine stummgeschalteten Chats',
    unmute: 'Stummschaltung aufheben',
    darkMode: 'Erscheinungsbild',
    darkModeHint: 'Wähle zwischen hellem und dunklem Design.',
    darkModeOptions: {
      SYSTEM: 'Automatisch (System)',
      LIGHT: 'Hell',
      DARK: 'Dunkel',
    },
    darkModeSaved: 'Erscheinungsbild gespeichert',
    emailDigest: 'E-Mail-Zusammenfassung',
    digestFrequency: 'Häufigkeit',
    digestHint: 'Erhalten Sie eine Zusammenfassung Ihrer Benachrichtigungen per E-Mail.',
    digestSaved: 'E-Mail-Einstellung gespeichert',
    digestFrequencies: {
      NONE: 'Deaktiviert',
      DAILY: 'Täglich',
      WEEKLY: 'Wöchentlich',
      BIWEEKLY: 'Alle 2 Wochen',
      MONTHLY: 'Monatlich',
    },
  },
  billing: {
    title: 'Jahresabrechnung',
    subtitle: 'Periodenbasierte Abrechnung der Elternstunden',
    createPeriod: 'Neuen Abrechnungszeitraum erstellen',
    periodName: 'Bezeichnung',
    periodNamePlaceholder: 'z.B. Schuljahr 2024/2025',
    startDate: 'Startdatum',
    endDate: 'Enddatum',
    create: 'Zeitraum erstellen',
    periodCreated: 'Abrechnungszeitraum erstellt',
    createError: 'Fehler beim Erstellen des Zeitraums',
    statusActive: 'Aktiv',
    statusClosed: 'Abgeschlossen',
    families: 'Familien',
    average: 'Durchschnitt',
    totalHours: 'Gesamt-Stunden',
    balanceCol: 'Saldo',
    targetCleaningCol: 'Soll Putz',
    cleaningBalanceCol: 'Saldo Putz',
    members: 'Familienmitglieder',
    roleParent: 'Eltern',
    roleChild: 'Kind',
    closePeriod: 'Abrechnung abschlie\u00dfen',
    closeConfirmTitle: 'Abrechnung abschlie\u00dfen',
    closeConfirmMessage: 'M\u00f6chten Sie die aktuelle Abrechnung wirklich abschlie\u00dfen? Der Bericht wird eingefroren und eine neue Periode wird automatisch erstellt.',
    closeConfirm: 'Abschlie\u00dfen',
    periodClosed: 'Abrechnung abgeschlossen',
    closeError: 'Fehler beim Abschlie\u00dfen',
    history: 'Vergangene Abrechnungen',
    viewReport: 'Bericht anzeigen',
  },
  errorReports: {
    title: 'Fehlermeldungen',
    subtitle: 'Automatisch erfasste Fehler verwalten',
    source: 'Quelle',
    errorType: 'Fehlertyp',
    message: 'Nachricht',
    stackTrace: 'Stack-Trace',
    occurrenceCount: 'Auftreten',
    firstSeen: 'Erstmals gesehen',
    lastSeen: 'Zuletzt gesehen',
    userAgent: 'Browser',
    requestUrl: 'Request-URL',
    statusNew: 'Neu',
    statusReported: 'Gemeldet',
    statusResolved: 'Behoben',
    statusIgnored: 'Ignoriert',
    createGithubIssue: 'GitHub Issue erstellen',
    githubIssueCreated: 'GitHub Issue erstellt',
    githubConfig: 'GitHub-Konfiguration',
    githubRepo: 'Repository (owner/repo)',
    githubPat: 'Personal Access Token',
    githubConfigSaved: 'GitHub-Konfiguration gespeichert',
    githubNotConfigured: 'GitHub ist noch nicht konfiguriert. Bitte Repository und PAT eingeben.',
    confirmDelete: 'M\u00f6chten Sie diesen Fehlerbericht wirklich l\u00f6schen?',
    deleted: 'Fehlerbericht gel\u00f6scht',
    statusUpdated: 'Status aktualisiert',
    noReports: 'Keine Fehlermeldungen vorhanden',
    all: 'Alle',
  },
  maintenance: {
    title: 'Wartungsarbeiten',
    defaultMessage: 'Das System wird gerade gewartet. Bitte versuchen Sie es später erneut.',
    loginAsAdmin: 'Als Admin anmelden',
  },
  error: {
    notFound: '404 - Seite nicht gefunden',
    notFoundMessage: 'Die angeforderte Seite existiert nicht oder wurde verschoben.',
    backHome: 'Zur\u00fcck zum Dashboard',
    unexpected: 'Ein unerwarteter Fehler ist aufgetreten. Bitte versuchen Sie es erneut.',
  },
  fundgrube: {
    title: 'Fundgrube',
    subtitle: 'Verlorene Gegenstände melden und wiederfinden',
    newItem: 'Gegenstand melden',
    editItem: 'Gegenstand bearbeiten',
    itemTitle: 'Bezeichnung',
    section: 'Schulbereich',
    noSection: 'Kein Bereich',
    allSections: 'Alle Bereiche',
    images: 'Bilder',
    chooseImages: 'Bilder auswählen',
    filesSelected: 'Datei(en) ausgewählt',
    empty: 'Keine Fundstücke vorhanden',
    postedBy: 'Gemeldet von',
    claimedBy: 'Abgeholt von',
    expiresAt: 'Wird entfernt am',
    claimed: 'Abgeholt',
    claimButton: 'Das gehört mir!',
    claimTitle: 'Gegenstand beanspruchen',
    claimDescription: 'Bitte bestätige, dass dieser Gegenstand dir gehört. Er wird nach einem Tag automatisch aus der Fundgrube entfernt.',
    claimComment: 'Kommentar (optional)',
    claimCommentPlaceholder: 'z.B. Beschreibung zur Identifikation...',
    claimSuccess: 'Du wirst benachrichtigt. Der Eintrag wird in einem Tag entfernt.',
    itemCreated: 'Fundstück wurde gemeldet',
    itemUpdated: 'Fundstück wurde aktualisiert',
    deleteConfirm: 'Möchtest du diesen Eintrag wirklich löschen?',
  },
  bookmarks: {
    title: 'Lesezeichen',
    subtitle: 'Gespeicherte Inhalte',
    empty: 'Noch keine Lesezeichen gespeichert',
    tabs: {
      all: 'Alle',
      posts: 'Beiträge',
      events: 'Termine',
      jobs: 'Jobs',
      wiki: 'Wiki',
    },
    types: {
      POST: 'Beitrag',
      EVENT: 'Termin',
      JOB: 'Job',
      WIKI_PAGE: 'Wiki-Seite',
    },
  },
  tasks: {
    title: 'Aufgaben',
    newTask: 'Neue Aufgabe',
    editTask: 'Aufgabe bearbeiten',
    taskTitle: 'Titel',
    description: 'Beschreibung',
    column: 'Spalte',
    assignee: 'Zugewiesen an',
    unassigned: 'Nicht zugewiesen',
    dueDate: 'Fälligkeitsdatum',
    dueDatePlaceholder: 'Datum wählen',
    titlePlaceholder: 'Aufgabe eingeben...',
    descriptionPlaceholder: 'Beschreibung (optional)',
    addHere: 'Hier hinzufügen',
    manageColumns: 'Spalten verwalten',
    newColumnName: 'Spaltenname',
    created: 'Aufgabe erstellt',
    saved: 'Aufgabe gespeichert',
    deleted: 'Aufgabe gelöscht',
    createError: 'Fehler beim Erstellen der Aufgabe',
    saveError: 'Fehler beim Speichern der Aufgabe',
    deleteError: 'Fehler beim Löschen der Aufgabe',
    moveError: 'Fehler beim Verschieben der Aufgabe',
    columnAdded: 'Spalte hinzugefügt',
    columnDeleted: 'Spalte gelöscht',
    columnAddError: 'Fehler beim Hinzufügen der Spalte',
    columnDeleteError: 'Spalte konnte nicht gelöscht werden',
    loadError: 'Aufgaben konnten nicht geladen werden',
    emptyBoard: 'Noch keine Aufgaben vorhanden. Erstelle eine neue Aufgabe, um loszulegen!',
    help: 'Hilfe',
    helpTitle: 'So funktioniert das Kanban-Board',
    helpIntro: 'Das Kanban-Board hilft euch, Aufgaben im Raum zu organisieren und den Fortschritt sichtbar zu machen.',
    helpWhatIsKanban: 'Was ist Kanban?',
    helpKanbanExplain: 'Kanban ist eine visuelle Methode zur Aufgabenverwaltung. Aufgaben werden als Karten dargestellt und durchlaufen Spalten von links nach rechts — z.B. von „Offen" über „In Arbeit" bis „Erledigt".',
    helpHowToUse: 'So benutzt du das Board',
    helpStep1: 'Erstelle eine neue Aufgabe mit dem „+" Button oder „Hier hinzufügen" in einer Spalte.',
    helpStep2: 'Ziehe Aufgaben per Drag & Drop von einer Spalte in eine andere, um den Status zu ändern.',
    helpStep3: 'Klicke auf eine Aufgabe, um sie zu bearbeiten, jemandem zuzuweisen oder ein Fälligkeitsdatum zu setzen.',
    helpStep4: 'Raumleitungen können über „Spalten verwalten" eigene Spalten hinzufügen oder entfernen.',
    helpTip: 'Überfällige Aufgaben werden rot markiert. Halte die Spalten übersichtlich — zu viele Aufgaben gleichzeitig „In Arbeit" bremsen den Fortschritt.',
    checklist: 'Checkliste',
    addChecklistItem: 'Punkt hinzufügen',
    checklistProgress: '{checked} von {total}',
    checklistItemAdded: 'Checklistenpunkt hinzugefügt',
    checklistItemDeleted: 'Checklistenpunkt gelöscht',
    checklistError: 'Fehler bei der Checkliste',
  },
  wiki: {
    title: 'Wiki',
    newPage: 'Neue Seite',
    editPage: 'Seite bearbeiten',
    deletePage: 'Seite löschen',
    deleteConfirm: 'Möchtest du diese Seite wirklich löschen? Unterseiten werden zu Hauptseiten.',
    pageTitle: 'Seitentitel',
    titlePlaceholder: 'Seitentitel eingeben...',
    content: 'Inhalt',
    contentPlaceholder: 'Seiteninhalt in Markdown eingeben...',
    parentPage: 'Übergeordnete Seite',
    noParent: 'Keine (Hauptseite)',
    preview: 'Vorschau',
    edit: 'Bearbeiten',
    history: 'Verlauf',
    versions: 'Versionen',
    versionBy: 'von {name}',
    viewVersion: 'Version ansehen',
    restoreVersion: 'Diese Version wiederherstellen',
    currentVersion: 'Aktuelle Version',
    noPages: 'Noch keine Wiki-Seiten vorhanden.',
    noPagesHint: 'Erstelle die erste Seite, um loszulegen.',
    search: 'Seiten durchsuchen...',
    searchResults: 'Suchergebnisse',
    noSearchResults: 'Keine Seiten gefunden.',
    lastEdited: 'Zuletzt bearbeitet von {name}',
    createdBy: 'Erstellt von {name}',
    created: 'Seite erstellt',
    saved: 'Seite gespeichert',
    deleted: 'Seite gelöscht',
    createError: 'Fehler beim Erstellen der Seite',
    saveError: 'Fehler beim Speichern der Seite',
    deleteError: 'Fehler beim Löschen der Seite',
    loadError: 'Wiki konnte nicht geladen werden',
    childPages: 'Unterseiten',
    backToTree: 'Zurück zur Übersicht',
  },
  pwa: {
    installTitle: 'MonteWeb installieren',
    installDescription: 'App auf dem Startbildschirm hinzufügen für schnelleren Zugriff',
    install: 'Installieren',
    offline: 'Keine Internetverbindung — Daten werden aus dem Cache angezeigt',
  },
  help: {
    contextHelp: 'Kontexthilfe',
    whatCanIDo: 'Was kann ich hier?',
    tips: 'Tipps',
    noContextHelp: 'Für diese Seite ist keine Kontexthilfe verfügbar.',
    openHandbook: 'Handbuch öffnen',
    nav: 'Hilfe',
    downloads: {
      title: 'Dokumente zum Download',
      handbookParent: 'Handbuch Eltern',
      handbookTeacher: 'Handbuch Lehrkräfte',
      handbookAdmin: 'Handbuch Administration',
      cheatsheetParent: 'Kurzanleitung Eltern',
      cheatsheetTeacher: 'Kurzanleitung Lehrkräfte',
      cheatsheetSectionAdmin: 'Kurzanleitung Bereichsleitung',
      cheatsheetSuperadmin: 'Kurzanleitung Superadmin',
    },
    pages: {
      dashboard: {
        title: 'Dashboard',
        parent: {
          action1: 'Neuigkeiten aus den Räumen deiner Kinder im Feed sehen',
          action2: 'Offene Formulare direkt ausfüllen',
          action3: 'Kommende Termine und Putzaktionen einsehen',
          action4: 'Benachrichtigungen über neue Nachrichten und Einladungen prüfen',
          tip1: 'Das Dashboard zeigt nur Inhalte aus Räumen, in denen du Mitglied bist.',
          tip2: 'Banner weisen dich auf wichtige anstehende Aufgaben hin.',
        },
        student: {
          action1: 'Feed-Beiträge aus deinen Räumen lesen',
          action2: 'Kommende Termine im Kalender sehen',
          action3: 'Offene Formulare ausfüllen',
          tip1: 'Du siehst nur Inhalte, die für Schüler freigegeben sind.',
        },
        teacher: {
          action1: 'Überblick über alle Räume und deren Aktivitäten erhalten',
          action2: 'Feed-Beiträge erstellen und verwalten',
          action3: 'Beitrittsanfragen für deine Räume bearbeiten',
          action4: 'Offene Formulare und Termine verwalten',
          tip1: 'Du kannst im Feed gezielt einzelne Nutzer ansprechen (Targeted Posts).',
          tip2: 'Über die Glocke siehst du neue Benachrichtigungen.',
        },
        sectionAdmin: {
          action1: 'Deinen Schulbereich überblicken',
          action2: 'Räume und Mitglieder deines Bereichs verwalten',
          action3: 'Feed und Termine einsehen',
          tip1: 'Im Bereichs-Admin-Panel findest du erweiterte Verwaltungsoptionen.',
        },
        admin: {
          action1: 'Systemstatus und Statistiken einsehen',
          action2: 'Alle Module und Bereiche verwalten',
          action3: 'Benutzer und Registrierungen überprüfen',
          action4: 'Fehlermeldungen und Audit-Log prüfen',
          tip1: 'Über das Admin-Menü erreichst du alle Verwaltungsbereiche.',
        },
      },
      rooms: {
        title: 'Meine Räume',
        parent: {
          action1: 'Räume deiner Kinder sehen und öffnen',
          action2: 'Weitere Räume über "Entdecken" finden und beitreten',
          tip1: 'Räume, in denen du Mitglied bist, werden hier angezeigt.',
        },
        student: {
          action1: 'Deine Gruppenräume sehen und öffnen',
          action2: 'Neue Räume entdecken und Beitritt anfragen',
          tip1: 'Bei archivierten Räumen kannst du noch lesen, aber nicht mehr schreiben.',
        },
        teacher: {
          action1: 'Deine Räume verwalten und neue erstellen',
          action2: 'Mitglieder und Einstellungen deiner Räume bearbeiten',
          action3: 'Beitrittsanfragen genehmigen oder ablehnen',
          tip1: 'Als Raumleitung hast du erweiterte Rechte in deinen Räumen.',
        },
        admin: {
          action1: 'Alle Räume der Schule sehen und verwalten',
          action2: 'Neue Räume erstellen und zuordnen',
          action3: 'Räume archivieren oder löschen',
          tip1: 'Im Admin-Bereich unter "Räume" findest du die vollständige Verwaltung.',
        },
      },
      roomDetail: {
        title: 'Raum-Detail',
        parent: {
          action1: 'Feed-Beiträge lesen und kommentieren',
          action2: 'Dateien und Fotos ansehen',
          action3: 'Diskussionen im Raum verfolgen',
          tip1: 'Einige Ordner sind nur für Eltern sichtbar (z.B. Elterninfos).',
        },
        student: {
          action1: 'Feed-Beiträge lesen und kommentieren',
          action2: 'An Diskussionen teilnehmen',
          tip1: 'Inhalte, die nur für Eltern bestimmt sind, siehst du nicht.',
        },
        teacher: {
          action1: 'Feed-Beiträge erstellen, bearbeiten und anpinnen',
          action2: 'Dateien hochladen und Ordner verwalten',
          action3: 'Fotobox-Threads erstellen und verwalten',
          action4: 'Mitglieder verwalten und Raumeinstellungen ändern',
          tip1: 'Du kannst Beiträge oben anpinnen, damit sie immer sichtbar sind.',
        },
        admin: {
          action1: 'Alle Raumfunktionen nutzen und verwalten',
          action2: 'Raumeinstellungen und Mitglieder bearbeiten',
          action3: 'Raum archivieren oder Mitglieder entfernen',
          action4: 'Alle Dateien und Fotobox-Inhalte verwalten',
          tip1: 'Als Admin hast du vollen Zugriff auf alle Raum-Funktionen.',
        },
      },
      discover: {
        title: 'Räume entdecken',
        action1: 'Verfügbare Räume durchsuchen und filtern',
        action2: 'Beitritt zu einem Raum anfragen',
        tip1: 'Die Raumleitung entscheidet über deine Beitrittsanfrage.',
      },
      family: {
        title: 'Familie',
        parent: {
          action1: 'Familienverbund erstellen oder beitreten',
          action2: 'Familienmitglieder einladen (Partner, Kinder)',
          action3: 'Elternstunden-Konto einsehen',
          action4: 'Einladungen annehmen oder ablehnen',
          tip1: 'Jedes Elternteil hat einen eigenen Familienverbund.',
          tip2: 'Kinder können mehreren Familien zugeordnet sein (getrennte Eltern).',
        },
        student: {
          action1: 'Deinen Familienverbund und Mitglieder sehen',
          action2: 'Familien-Einladungen annehmen oder ablehnen',
          tip1: 'Du kannst mehreren Familien zugeordnet sein.',
        },
        admin: {
          action1: 'Alle Familien der Schule einsehen',
          action2: 'Familien verwalten und Stunden korrigieren',
          tip1: 'Im Admin-Bereich unter "Familien" findest du die vollständige Verwaltung.',
        },
      },
      messages: {
        title: 'Nachrichten',
        parent: {
          action1: 'Direktnachrichten an Lehrkräfte senden',
          action2: 'Bilder in Nachrichten versenden',
          action3: 'Auf Nachrichten antworten (Reply-Funktion)',
          tip1: 'Ob du andere Eltern anschreiben kannst, hängt von den Kommunikationsregeln ab.',
        },
        student: {
          action1: 'Nachrichten an Lehrkräfte senden',
          action2: 'Auf Nachrichten antworten',
          tip1: 'Die Kommunikationsregeln bestimmen, wen du anschreiben kannst.',
        },
        teacher: {
          action1: 'Nachrichten an Eltern und Schüler senden',
          action2: 'Gruppengespräche starten',
          action3: 'Bilder teilen und auf Nachrichten antworten',
          tip1: 'Du kannst immer mit Eltern und Schülern kommunizieren.',
        },
        admin: {
          action1: 'Alle Kommunikationsfunktionen nutzen',
          action2: 'Kommunikationsregeln in den Einstellungen anpassen',
          tip1: 'Unter Admin → Module kannst du Kommunikationsregeln konfigurieren.',
        },
      },
      jobs: {
        title: 'Jobbörse / Elternstunden',
        parent: {
          action1: 'Verfügbare Jobs durchsuchen und sich bewerben',
          action2: 'Eigene Bewerbungen und zugewiesene Jobs einsehen',
          action3: 'Geleistete Stunden im Familienkonto nachverfolgen',
          tip1: 'Jobs werden deinem Familien-Stundenkonto gutgeschrieben.',
          tip2: 'Du kannst dich auch für Jobs in anderen Räumen bewerben.',
        },
        teacher: {
          action1: 'Jobs für deinen Raum erstellen und verwalten',
          action2: 'Bewerbungen prüfen und zuweisen',
          action3: 'Geleistete Stunden bestätigen',
          tip1: 'Erstelle Jobs mit klaren Beschreibungen und Zeitangaben.',
        },
        admin: {
          action1: 'Alle Jobs der Schule einsehen und verwalten',
          action2: 'Stunden-Berichte und Statistiken abrufen',
          action3: 'Jahresabrechnung erstellen und exportieren',
          tip1: 'Im Admin-Bereich findest du den vollständigen Job-Report.',
        },
      },
      cleaning: {
        title: 'Putz-Orga',
        parent: {
          action1: 'Verfügbare Putztermine einsehen und eintragen',
          action2: 'Eigene Putztermine im Kalender sehen',
          action3: 'Am Putztag per QR-Code einchecken',
          tip1: 'Putzstunden werden gesondert auf deinem Familienkonto erfasst.',
          tip2: 'Putzaktionen erscheinen auch im Kalender und als Feed-Banner.',
        },
        teacher: {
          action1: 'Putztermine und Eintragungen einsehen',
          action2: 'Putzaktionen im Kalender verfolgen',
          tip1: 'Die Putz-Organisation wird über den Admin-Bereich verwaltet.',
        },
        admin: {
          action1: 'Putzaktionen erstellen (einmalig oder wiederkehrend)',
          action2: 'Putzslots und Familien-Zuordnungen verwalten',
          action3: 'QR-Codes generieren und Check-in-Status prüfen',
          tip1: 'Putzaktionen erzeugen automatisch Kalendereinträge und Jobs.',
        },
      },
      calendar: {
        title: 'Kalender',
        parent: {
          action1: 'Termine deiner Räume und der Schule einsehen',
          action2: 'Zu Terminen zu- oder absagen (RSVP)',
          tip1: 'Abgesagte Termine erscheinen als Info im Feed.',
        },
        student: {
          action1: 'Schultermine und Raumtermine sehen',
          action2: 'Zu Terminen zu- oder absagen',
          tip1: 'Feiertage und Ferien werden farbig markiert.',
        },
        teacher: {
          action1: 'Raum-Termine erstellen und verwalten',
          action2: 'Bereichs-Termine für deinen Schulbereich erstellen',
          action3: 'Zu- und Absagen der Teilnehmer einsehen',
          tip1: 'Abgesagte Termine erzeugen automatisch Feed-Beiträge.',
        },
        admin: {
          action1: 'Schulweite Termine erstellen',
          action2: 'Alle Termin-Typen verwalten (Raum, Bereich, Schule)',
          action3: 'Feiertage und Ferien konfigurieren',
          tip1: 'Unter Admin → Design kannst du das Bundesland für Feiertage einstellen.',
        },
      },
      forms: {
        title: 'Formulare',
        parent: {
          action1: 'Offene Formulare ausfüllen und absenden',
          action2: 'Eigene Antworten einsehen',
          tip1: 'Formulare können Umfragen oder Einverständniserklärungen sein.',
        },
        student: {
          action1: 'Für dich freigegebene Formulare ausfüllen',
          tip1: 'Nicht alle Formulare sind für Schüler sichtbar.',
        },
        teacher: {
          action1: 'Formulare erstellen (Umfragen, Einverständnis)',
          action2: 'Ergebnisse und Antworten auswerten',
          action3: 'Formulare als CSV oder PDF exportieren',
          tip1: 'Du kannst Formulare für einzelne Räume oder ganze Bereiche erstellen.',
        },
        admin: {
          action1: 'Alle Formulare einsehen und verwalten',
          action2: 'Schulweite Formulare erstellen',
          action3: 'Ergebnisse exportieren (CSV/PDF)',
          tip1: 'Formulare können mehrere Schulbereiche gleichzeitig abdecken.',
        },
      },
      fundgrube: {
        title: 'Fundgrube',
        action1: 'Gefundene Gegenstände melden und Fotos hochladen',
        action2: 'Nach verlorenen Gegenständen suchen',
        action3: 'Eigene Gegenstände beanspruchen ("Das gehört mir!")',
        tip1: 'Beanspruchte Gegenstände werden nach 24 Stunden automatisch entfernt.',
      },
      profile: {
        title: 'Profil',
        action1: 'Persönliche Daten und Profilbild bearbeiten',
        action2: 'Passwort ändern',
        action3: 'Benachrichtigungs-Einstellungen anpassen',
        action4: 'Datenexport anfordern (DSGVO)',
        action5: 'Konto löschen mit 14-Tage-Frist',
        action6: 'Stummgeschaltete Chats verwalten',
        tip1: 'Du kannst dein Profilbild per Klick auf den Avatar ändern.',
        tip2: 'Der Datenexport enthält Daten aus allen Modulen.',
      },
      adminDashboard: {
        title: 'Admin-Dashboard',
        action1: 'Systemstatistiken und Aktivitätsübersicht einsehen',
        action2: 'Neue Registrierungen genehmigen oder ablehnen',
        action3: 'Fehlermeldungen und Systemstatus prüfen',
        tip1: 'Hier siehst du auf einen Blick den Zustand des gesamten Systems.',
      },
      adminUsers: {
        title: 'Benutzerverwaltung',
        action1: 'Benutzer suchen, filtern und verwalten',
        action2: 'Rollen zuweisen und ändern',
        action3: 'Benutzer aktivieren, deaktivieren oder löschen',
        action4: 'Neue Registrierungen genehmigen',
        action5: 'DSGVO-Daten exportieren und Konten löschen (DSGVO-Tab)',
        tip1: 'Benutzer können mehrere Rollen haben und zwischen ihnen wechseln.',
        tip2: 'Im DSGVO-Tab kannst du Benutzerdaten exportieren und Löschungen verwalten.',
      },
      adminRooms: {
        title: 'Raumverwaltung',
        action1: 'Alle Räume der Schule einsehen und verwalten',
        action2: 'Neue Räume erstellen und Schulbereichen zuordnen',
        action3: 'Räume archivieren oder Mitglieder verwalten',
        tip1: 'KLASSE-Räume erhalten automatisch einen Standard-Ordner.',
      },
      adminSections: {
        title: 'Schulbereiche',
        action1: 'Schulbereiche (Krippe bis Oberstufe) verwalten',
        action2: 'Bereichs-Admins zuweisen',
        tip1: 'Bereiche bilden die organisatorische Grundstruktur der Schule.',
      },
      adminFamilies: {
        title: 'Familienverwaltung',
        action1: 'Alle Familienverbünde einsehen',
        action2: 'Stunden korrigieren und Familien verwalten',
        action3: 'Stunden-Befreiung aktivieren/deaktivieren',
        tip1: 'Befreite Familien erscheinen nicht in der Stundenbilanz.',
      },
      adminModules: {
        title: 'Modulverwaltung',
        action1: 'Module aktivieren oder deaktivieren',
        action2: 'Kommunikationsregeln und Systemeinstellungen konfigurieren',
        tip1: 'Deaktivierte Module werden aus der Navigation entfernt.',
      },
      adminCleaning: {
        title: 'Putz-Orga Verwaltung',
        action1: 'Putzaktionen erstellen (einmalig mit Datum oder wiederkehrend)',
        action2: 'Slots und Zuordnungen verwalten',
        action3: 'Check-in-Status überwachen',
        action4: 'QR-Codes für Putztermine generieren',
        tip1: 'Putzaktionen erzeugen automatisch Kalender-Events und Jobs.',
      },
      adminTheme: {
        title: 'Design & Einstellungen',
        action1: 'Schulfarben und Logo anpassen',
        action2: 'Bundesland für Feiertagsberechnung einstellen',
        action3: 'Schulferien konfigurieren',
        tip1: 'Änderungen am Design werden sofort für alle Nutzer sichtbar.',
      },
      adminBilling: {
        title: 'Jahresabrechnung',
        action1: 'Abrechnungsperioden erstellen und verwalten',
        action2: 'Familien-Stundenbilanz einsehen',
        action3: 'Berichte als PDF exportieren',
        tip1: 'Geschlossene Perioden können nicht mehr verändert werden.',
      },
      adminSettings: {
        title: 'Einstellungen',
        action1: 'Sprache, Registrierung und Stundenkonfiguration verwalten',
        action2: 'Bundesland und Schulferien einstellen',
        action3: 'Bestätigungspflicht für Elternstunden konfigurieren',
        tip1: 'Änderungen an Sprache und Registrierung gelten sofort für alle Nutzer.',
      },
      adminPrivacy: {
        title: 'Datenschutz-Verwaltung',
        action1: 'Datenschutzerklärung und Nutzungsbedingungen bearbeiten',
        action2: 'Ausstehende Kontoloeschungen einsehen',
        action3: 'Datenzugriffs-Protokoll prüfen',
        tip1: 'Änderungen an Datenschutztexten erfordern ggf. eine erneute Zustimmung der Nutzer.',
      },
      privacyPolicy: {
        title: 'Datenschutzerklärung',
        action1: 'Datenschutzerklärung der Schule lesen',
        action2: 'Informationen zu Datenverarbeitung und Rechten einsehen',
        tip1: 'Die Datenschutzerklärung ist auch ohne Anmeldung zugänglich.',
      },
      terms: {
        title: 'Nutzungsbedingungen',
        action1: 'Nutzungsbedingungen der Schule lesen',
        action2: 'Nutzungsbedingungen akzeptieren (nach Anmeldung)',
        tip1: 'Neue Versionen der Nutzungsbedingungen müssen erneut akzeptiert werden.',
      },
      sectionAdmin: {
        title: 'Bereichsverwaltung',
        action1: 'Räume des eigenen Bereichs verwalten',
        action2: 'Mitglieder des Bereichs einsehen',
        action3: 'Bereichsüberblick und Statistiken ansehen',
        tip1: 'Du siehst nur die Räume und Mitglieder deines Schulbereichs.',
      },
    },
    handbook: {
      title: 'Handbuch',
      subtitle: 'Anleitung für {role}',
      searchPlaceholder: 'Hilfethemen durchsuchen...',
      noResults: 'Keine passenden Hilfethemen gefunden.',
      roleLabel: {
        PARENT: 'Eltern',
        STUDENT: 'Schüler',
        TEACHER: 'Lehrkräfte',
        SECTION_ADMIN: 'Bereichsadmins',
        SUPERADMIN: 'Administratoren',
      },
      parent: {
        gettingStarted: {
          title: 'Erste Schritte',
          login: {
            title: 'Anmeldung & Registrierung',
            content: 'Öffne MonteWeb im Browser und melde dich mit deiner E-Mail-Adresse und deinem Passwort an. Falls deine Schule Single Sign-On (SSO) nutzt, kannst du alternativ den Button „Mit SSO anmelden" verwenden.',
            content2: 'Falls du noch kein Konto hast: Klicke auf „Registrieren", fülle die Pflichtfelder aus (E-Mail, Vorname, Nachname, Passwort) und achte auf die Passwort-Stärkeanzeige. Je nach Schuleinstellung muss ein Administrator deinen Zugang nach der Registrierung freigeben. Über „Passwort vergessen?" auf der Login-Seite kannst du ein neues Passwort anfordern.',
          },
          dashboard: {
            title: 'Das Dashboard',
            content: 'Nach der Anmeldung siehst du das Dashboard — deine persönliche Startseite. Oben erscheinen kontextabhängige Systembanner, z.B. Erinnerungen an anstehende Putztermine oder Hinweise auf abgesagte Veranstaltungen. Darunter findest du das Elternstunden-Widget mit dem aktuellen Stand deiner regulären Stunden und Putzstunden.',
            content2: 'Außerdem werden offene Formulare (Umfragen, Einverständniserklärungen) als Widget angezeigt — bis zu 5 unbearbeitete Formulare mit Direktlink. Unter den Widgets erscheint der Haupt-Feed mit Beiträgen aus allen Räumen, in denen du oder deine Kinder Mitglied sind.',
          },
          navigation: {
            title: 'Navigation',
            content: 'Am Desktop findest du links eine Seitenleiste (Sidebar) mit allen Menüpunkten: Dashboard, Räume, Familie, Nachrichten, Jobs, Putzplan, Kalender, Formulare und Fundgrube. Auf dem Smartphone erscheint stattdessen eine untere Navigationsleiste mit den wichtigsten Punkten und einem „Mehr"-Button für weitere Optionen. In der Kopfzeile siehst du das Schullogo, dein Rollen-Badge (farbcodiert), den Sprachumschalter (DE/EN), die Benachrichtigungsglocke und dein Profilmenü.',
          },
          profile: {
            title: 'Profil bearbeiten',
            content: 'Unter „Profil" (Klick auf deinen Avatar oben rechts) kannst du dein Profilbild hochladen, Vorname, Nachname und Telefonnummer bearbeiten und dein Passwort ändern. Falls du mehrere Rollen hast (z.B. Lehrkraft und Elternteil), findest du hier auch den Rollenwechsel.',
            content2: 'Am unteren Rand der Profilseite findest du DSGVO-Funktionen: „Daten exportieren" lädt alle persönlichen Daten als JSON herunter. „Konto löschen" anonymisiert deine Daten unwiderruflich. Du kannst außerdem Push-Benachrichtigungen aktivieren, um auch unterwegs über neue Nachrichten informiert zu werden.',
          },
        },
        family: {
          title: 'Familienverbund',
          create: {
            title: 'Familie erstellen',
            content: 'Unter „Familie" in der Navigation kannst du deinen Familienverbund erstellen. Gib einen Familiennamen ein — du wirst automatisch als Familienoberhaupt eingetragen. Der Familienverbund ist die zentrale Abrechnungseinheit für deine Elternstunden.',
            content2: 'Wichtig: Jedes Elternteil gehört zu genau einem Familienverbund. Kinder können mehreren Familien zugeordnet sein (z.B. bei getrennten Eltern). Das Stundenkonto ist immer an den Familienverbund gekoppelt, nicht an die Einzelperson.',
          },
          invite: {
            title: 'Mitglieder einladen',
            content: 'Lade deinen Partner oder deine Kinder ein: Klicke auf „Mitglied einladen", suche die Person nach Name oder E-Mail und wähle die Rolle (Elternteil oder Kind). Die eingeladene Person erhält eine Benachrichtigung und kann annehmen oder ablehnen.',
            content2: 'Beispiel: Du lädst deinen Partner als „Elternteil" ein — er kann dann ebenfalls auf das gemeinsame Stundenkonto zugreifen. Dein Kind lädst du als „Kind" ein — es wird dem Familienverbund zugeordnet, ohne eigene Stundenrechte zu haben.',
          },
          hours: {
            title: 'Stundenkonto',
            content: 'Dein Familien-Stundenkonto hat zwei getrennte Bereiche: Reguläre Elternstunden (aus der Jobbörse) und Putzstunden (aus der Putz-Organisation). Beide werden auf dem Dashboard als Widget mit Fortschrittsanzeige dargestellt.',
            content2: 'Im Detail siehst du unter Familie → Stunden eine Aufschlüsselung aller geleisteten Stunden. Am Jahresende erstellt die Schulverwaltung eine Jahresabrechnung, die alle Stunden zusammenfasst. Familien können von der Stundenpflicht befreit werden — in diesem Fall erscheint ein entsprechender Hinweis.',
          },
        },
        rooms: {
          title: 'Räume',
          overview: {
            title: 'Raumübersicht',
            content: 'Unter „Meine Räume" siehst du alle Räume als Kacheln mit Raum-Avatar, Name und Typ (Klasse, Gruppe, Projekt). Typischerweise bist du in den Gruppen deiner Kinder Mitglied.',
            content2: 'Über den Button „Räume entdecken" findest du weitere Räume. Offene Räume kannst du direkt beitreten, bei geschlossenen Räumen sendest du eine Beitrittsanfrage mit optionaler Nachricht. Die Raumleitung entscheidet dann über deinen Beitritt.',
          },
          feed: {
            title: 'Feed & Diskussionen',
            content: 'In jedem Raum findest du ein Info-Board mit aktuellen Beiträgen der Lehrkräfte und Ankündigungen. Angepinnte Beiträge stehen immer oben. Du kannst jeden Beitrag kommentieren.',
            content2: 'Im Tab „Diskussionen" findest du themenbezogene Threads. Manche Threads sind nur für Eltern sichtbar (z.B. organisatorische Absprachen), andere für alle Mitglieder. Du kannst auf alle für dich sichtbaren Threads antworten.',
          },
          files: {
            title: 'Dateien',
            content: 'Im Tab „Dateien" findest du Dokumente in einer Ordnerstruktur mit Breadcrumb-Navigation. Ordner haben eine Sichtbarkeitssteuerung: Manche sind nur für Eltern zugänglich (orange Tag „Nur Eltern"), andere für alle.',
            content2: 'Du kannst selbst Dateien hochladen — Ordner, die du erstellst, werden automatisch auf „Nur Eltern" gesetzt. Zum Herunterladen klicke auf den Dateinamen. Die Lehrkraft entscheidet, welche Ordner welcher Zielgruppe zur Verfügung stehen.',
          },
          fotobox: {
            title: 'Fotobox',
            content: 'Die Fotobox enthält Foto-Galerien (Threads) zu Veranstaltungen und Projekten. Du kannst Bilder ansehen, in der Lightbox durchblättern und eigene Fotos hochladen (bis zu 20 pro Upload). Thumbnails werden automatisch generiert. Je nach Raumeinstellung darfst du Bilder in bestehende Threads hochladen oder nur ansehen.',
          },
        },
        communication: {
          title: 'Kommunikation',
          messages: {
            title: 'Direktnachrichten',
            content: 'Unter „Nachrichten" findest du deine Konversationen. Klicke auf „Neue Nachricht", suche den Empfänger und schreibe los. Du kannst jederzeit Nachrichten an Lehrkräfte senden.',
            content2: 'Du kannst Bilder versenden und auf einzelne Nachrichten antworten (Reply-Funktion). Ob du andere Eltern anschreiben kannst, hängt von den Kommunikationsregeln der Schule ab — standardmäßig ist Eltern-zu-Eltern-Kommunikation gesperrt, kann aber vom Admin freigegeben werden.',
          },
          notifications: {
            title: 'Benachrichtigungen',
            content: 'Die Glocke in der Kopfzeile zeigt die Anzahl ungelesener Benachrichtigungen. Klicke darauf, um alle Benachrichtigungen zu sehen: neue Nachrichten, Feed-Beiträge, Familien-Einladungen, Job-Updates, Formular-Hinweise und Termin-Änderungen. Du kannst Push-Benachrichtigungen im Profil aktivieren, um auch bei geschlossenem Browser informiert zu werden.',
          },
          feed: {
            title: 'Feed-Benachrichtigungen',
            content: 'Neue Feed-Beiträge und Kommentare auf deine Beiträge erscheinen als Benachrichtigungen. Abgesagte Termine erzeugen automatisch einen Feed-Beitrag, damit du sofort informiert bist. Systembanner auf dem Dashboard weisen dich auf besonders wichtige Dinge hin (z.B. bevorstehende Putztermine).',
          },
        },
        services: {
          title: 'Dienste & Funktionen',
          jobs: {
            title: 'Jobbörse (Elternstunden)',
            content: 'In der Jobbörse findest du verfügbare Aufgaben. Öffne einen Job, lies die Beschreibung und klicke auf „Bewerben" (optional mit Nachricht). Der Ablauf ist: Offen → Beworben → Zugewiesen → Gestartet → Abgeschlossen → Bestätigt.',
            content2: 'Nachdem du zugewiesen wurdest, startest du die Arbeit und meldest sie als erledigt. Die Lehrkraft bestätigt die Stunden, die dann deinem Familienkonto gutgeschrieben werden. Deine aktuellen Bewerbungen und zugewiesenen Jobs siehst du im Tab „Meine Jobs".',
          },
          cleaning: {
            title: 'Putz-Orga',
            content: 'Im Putzplan siehst du alle verfügbaren Putztermine — wiederkehrende (z.B. jeden Mittwoch) und einmalige Putzaktionen (z.B. Großputz am 15.03.). Wähle einen freien Termin und klicke auf „Anmelden". Du kannst dich auch wieder abmelden oder einen Tausch anbieten.',
            content2: 'Am Putztag scannst du den QR-Code am Putzort mit deinem Smartphone zum Check-in. Nach getaner Arbeit scannst du erneut zum Check-out. Die Putzstunden werden automatisch deinem Familien-Sonderkonto gutgeschrieben. Putztermine erscheinen auch im Kalender und als Banner auf dem Dashboard.',
          },
          calendar: {
            title: 'Kalender',
            content: 'Der Kalender zeigt Termine chronologisch nach Monaten gegliedert. Du siehst Raum-Termine, Bereichs-Termine und schulweite Events. Nutze die Monatsnavigation (Zurück/Heute/Vor) zum Blättern.',
            content2: 'Für jeden Termin kannst du per RSVP zusagen (grün), „Vielleicht" wählen (gelb) oder absagen (rot). Wird ein Termin abgesagt, erscheint ein Info-Beitrag im Feed. Feiertage werden rot markiert, Schulferien orange — so hast du immer den Überblick.',
          },
          forms: {
            title: 'Formulare',
            content: 'Offene Formulare findest du unter „Formulare" sowie als Widget auf dem Dashboard (bis zu 5 unbearbeitete). Es gibt Umfragen (blaues Tag) und Einverständniserklärungen (oranges Tag).',
            content2: 'Öffne ein Formular, beantworte die Fragen und klicke auf „Absenden". Bei Einverständniserklärungen triffst du eine explizite Ja/Nein-Entscheidung. Bereits beantwortete Formulare sind mit einem grünen Häkchen markiert. Manche Formulare haben eine Frist — achte auf das angezeigte Datum.',
          },
          fundgrube: {
            title: 'Fundgrube',
            content: 'Die schulweite Fundgrube hilft beim Wiederfinden verlorener Gegenstände. Du kannst nach Schulbereich filtern und gefundene Sachen durchsuchen.',
            content2: 'So funktioniert es: Findest du etwas, klicke auf „Neuer Eintrag", beschreibe den Gegenstand und lade ein Foto hoch. Erkennst du deinen verlorenen Gegenstand, klicke auf „Das gehört mir!" — der Eintrag wird dann als beansprucht markiert und nach 24 Stunden automatisch entfernt.',
          },
        },
      },
      student: {
        gettingStarted: {
          title: 'Erste Schritte',
          login: {
            title: 'Anmeldung',
            content: 'Öffne MonteWeb im Browser und melde dich mit deiner E-Mail und deinem Passwort an. Falls du noch kein Konto hast, registriere dich über die Anmeldeseite. Dein Zugang muss möglicherweise erst von einem Administrator freigeschaltet werden.',
          },
          dashboard: {
            title: 'Das Dashboard',
            content: 'Nach der Anmeldung siehst du das Dashboard mit dem Feed deiner Räume (z.B. Klassengruppe), anstehenden Terminen und offenen Formularen. Du siehst nur Inhalte, die für Schüler freigegeben sind — manche Bereiche sind nur für Eltern oder Lehrkräfte sichtbar.',
          },
          profile: {
            title: 'Profil bearbeiten',
            content: 'Unter „Profil" kannst du dein Profilbild ändern und deine Daten aktualisieren. Du findest dort auch die Möglichkeit, Push-Benachrichtigungen zu aktivieren und einen Datenexport (DSGVO) anzufordern.',
          },
        },
        rooms: {
          title: 'Räume',
          overview: {
            title: 'Meine Räume',
            content: 'Unter „Meine Räume" siehst du alle Gruppen und Projekte, in denen du Mitglied bist. Über „Entdecken" kannst du weitere offene Räume finden und beitreten oder eine Beitrittsanfrage stellen.',
          },
          feed: {
            title: 'Feed & Beiträge',
            content: 'Im Feed deines Raums findest du Ankündigungen und Beiträge der Lehrkräfte. Du kannst alle Beiträge kommentieren. Angepinnte Beiträge stehen immer ganz oben.',
          },
          files: {
            title: 'Dateien & Fotobox',
            content: 'Im Dateien-Tab findest du Dokumente und Materialien. Manche Ordner sind nur für Eltern sichtbar — du siehst nur die für Schüler freigegebenen Inhalte. In der Fotobox kannst du Fotos von Veranstaltungen ansehen und je nach Berechtigung eigene hochladen.',
          },
          discussions: {
            title: 'Diskussionen & Chat',
            content: 'Im Tab „Diskussionen" kannst du an Threads teilnehmen, die für Schüler sichtbar sind. Falls der Chat im Raum aktiviert ist, findest du dort auch einen Echtzeit-Chat für den schnellen Austausch mit anderen Raummitgliedern.',
          },
        },
        tools: {
          title: 'Funktionen',
          calendar: {
            title: 'Kalender',
            content: 'Im Kalender siehst du Termine deiner Räume und der Schule. Du kannst zu Terminen zu- oder absagen (RSVP). Feiertage werden rot markiert, Schulferien orange.',
          },
          forms: {
            title: 'Formulare',
            content: 'Falls Formulare für Schüler freigegeben sind, findest du sie unter „Formulare" und als Widget auf dem Dashboard. Öffne ein Formular, beantworte die Fragen und sende es ab.',
          },
          messages: {
            title: 'Nachrichten',
            content: 'Du kannst Nachrichten an Lehrkräfte senden. Ob du auch anderen Schülern schreiben kannst, hängt von den Kommunikationsregeln der Schule ab. Du kannst Bilder versenden und die Reply-Funktion nutzen.',
          },
        },
      },
      teacher: {
        gettingStarted: {
          title: 'Erste Schritte',
          login: {
            title: 'Anmeldung',
            content: 'Melde dich mit deinen Zugangsdaten an. Als Lehrkraft hast du erweiterte Rechte: Du kannst Feed-Beiträge erstellen, Räume verwalten, Termine und Formulare anlegen sowie Jobs für Elternstunden erstellen.',
          },
          dashboard: {
            title: 'Das Dashboard',
            content: 'Das Dashboard zeigt dir eine Übersicht über die Aktivitäten in deinen Räumen, anstehende Termine und offene Aufgaben. Du siehst den Post-Composer, mit dem du schnell einen neuen Beitrag erstellen kannst.',
            content2: 'Über die Benachrichtigungsglocke siehst du neue Beitrittsanfragen für deine Räume, Bewerbungen auf deine Jobs und andere Neuigkeiten. Offene Formulare werden ebenfalls als Widget angezeigt.',
          },
        },
        rooms: {
          title: 'Raumverwaltung',
          manage: {
            title: 'Räume verwalten',
            content: 'Als Raumleitung (LEADER) hast du volle Kontrolle über deinen Raum: Raum-Avatar hochladen, öffentliche Beschreibung bearbeiten, Beitrittspolitik festlegen (Offen/Anfrage/Nur Einladung), Diskussionsmodus wählen und Chat aktivieren/deaktivieren.',
            content2: 'Du kannst den Raum auch archivieren (Soft-Delete, rückgängig machbar). Neue Räume erstellst du über „Räume" → „Neuer Raum" mit Name, Typ (Klasse/Gruppe/Projekt) und Schulbereich.',
          },
          feed: {
            title: 'Feed-Beiträge',
            content: 'Im Info-Board des Raums erstellst du Beiträge mit optionalem Titel und Inhalt. Du kannst Beiträge anpinnen, damit sie immer oben erscheinen, und Kommentare moderieren.',
            content2: 'Gezielte Beiträge (Targeted Posts): Du kannst Beiträge erstellen, die nur für bestimmte Nutzer sichtbar sind — z.B. eine persönliche Nachricht an einzelne Eltern direkt im Feed des Raums.',
          },
          files: {
            title: 'Dateien verwalten',
            content: 'Lade Dokumente hoch und erstelle Ordner mit unterschiedlicher Sichtbarkeit: „Alle" (für alle Raummitglieder), „Nur Eltern" oder „Nur Schüler". So kannst du z.B. Elterninformationen getrennt von Unterrichtsmaterialien ablegen.',
            content2: 'Beispiel: Erstelle einen Ordner „Elternbriefe" mit Sichtbarkeit „Nur Eltern" für organisatorische Dokumente und einen Ordner „Arbeitsblätter" mit Sichtbarkeit „Alle" für den Unterricht. Jeder sieht nur die für seine Rolle freigegebenen Inhalte.',
          },
          fotobox: {
            title: 'Fotobox',
            content: 'Erstelle Foto-Threads (Alben) für Veranstaltungen, Projekte oder den Schulalltag. Wähle dabei die Sichtbarkeit: „Alle", „Nur Eltern" oder „Nur Schüler". Bilder werden automatisch mit Thumbnails versehen.',
            content2: 'Du kannst die Standard-Berechtigung für Raummitglieder festlegen: Nur Ansehen, Bilder Posten oder Threads Erstellen. Pro Upload sind maximal 20 Bilder möglich. In der Lightbox können Bilder in Vollbild durchgeblättert werden.',
          },
          discussions: {
            title: 'Diskussionen',
            content: 'Erstelle themenbezogene Diskussions-Threads mit Titel, Inhalt und Zielgruppe (Alle, nur Eltern, nur Schüler). Threads können archiviert oder gelöscht werden. Alle berechtigten Raummitglieder können antworten.',
          },
          members: {
            title: 'Mitgliederverwaltung',
            content: 'Im Tab „Mitglieder" siehst du alle Raummitglieder gruppiert nach Leitung, Familien und Einzelpersonen. Du kannst neue Mitglieder einzeln oder als Familie hinzufügen, Beitrittsanfragen genehmigen/ablehnen und Mitglieder entfernen.',
          },
        },
        communication: {
          title: 'Kommunikation',
          messages: {
            title: 'Direktnachrichten',
            content: 'Als Lehrkraft kannst du immer Nachrichten an Eltern und Schüler senden. Nutze die Suchfunktion, um Empfänger zu finden, und versende Textnachrichten mit optionalen Bildern.',
            content2: 'Die Reply-Funktion ermöglicht übersichtliche Gesprächsverläufe: Wische oder klicke auf eine Nachricht, um direkt darauf zu antworten. Im Raum-Chat (Tab „Chat") kannst du in Echtzeit mit allen Raummitgliedern kommunizieren.',
          },
          feed: {
            title: 'Feed nutzen',
            content: 'Der Feed ist dein zentraler Informationskanal. Erstelle Beiträge mit Ankündigungen, pinne wichtige Infos an und kommentiere. Abgesagte Termine und Systembanner erscheinen automatisch im Feed.',
          },
        },
        tools: {
          title: 'Werkzeuge',
          calendar: {
            title: 'Kalender',
            content: 'Erstelle Termine auf Raum-Ebene (als LEADER) oder Bereichs-Ebene (als Lehrkraft). Wähle: Titel, Beschreibung, Ort, Datum/Uhrzeit, ganztägig oder nicht, und optionale Wiederholung (täglich/wöchentlich/monatlich/jährlich).',
            content2: 'Teilnehmer können per RSVP zusagen, „Vielleicht" wählen oder absagen. Sagst du einen Termin ab, wird automatisch ein Feed-Beitrag für alle Betroffenen erstellt. Löschst du einen Termin, erhalten nur die Zusager eine Benachrichtigung. Du kannst auch Jobs direkt mit Terminen verknüpfen.',
          },
          forms: {
            title: 'Formulare',
            content: 'Erstelle Umfragen (SURVEY) oder Einverständniserklärungen (CONSENT). Wähle den Geltungsbereich: Raum, Bereich (auch mehrere gleichzeitig!) oder Schule. Verfügbare Fragetypen: Freitext, Einzelauswahl, Mehrfachauswahl, Bewertungsskala und Ja/Nein.',
            content2: 'Formulare durchlaufen den Lebenszyklus: Entwurf → Veröffentlicht → Geschlossen → Archiviert. Unter „Ergebnisse" siehst du Rücklaufquote, Einzelantworten und Zusammenfassungen. Ergebnisse lassen sich als CSV oder PDF exportieren. Anonyme Formulare zeigen keine Nutzerzuordnung.',
          },
          jobs: {
            title: 'Jobbörse',
            content: 'Erstelle Jobs mit Titel, Beschreibung, Stundenwert (z.B. 2,0 Stunden), maximalem Teilnehmerlimit und optionaler Verknüpfung zu einem Kalendertermin.',
            content2: 'Der Workflow: Eltern bewerben sich → du prüfst und weist zu → der Elternteil startet und schließt ab → du bestätigst die Stunden (je nach Einstellung automatisch oder manuell). Die bestätigten Stunden werden dem Familienkonto gutgeschrieben.',
          },
        },
      },
      admin: {
        overview: {
          title: 'Überblick',
          dashboard: {
            title: 'Admin-Dashboard',
            content: 'Das Admin-Dashboard zeigt auf einen Blick: Systemstatistiken, aktuelle Aktivitäten, ausstehende Registrierungen und Fehlermeldungen. Von den Kacheln aus erreichst du direkt alle Verwaltungsbereiche.',
            content2: 'Kacheln im Dashboard: Benutzerverwaltung, Raumverwaltung, Schulbereiche, Familien, Module, Putz-Orga, Stunden-Berichte, Jahresabrechnung, Design, Einstellungen und Fehlermeldungen. Jede Kachel zeigt eine Kennzahl (z.B. Anzahl Benutzer, offene Registrierungen).',
          },
          navigation: {
            title: 'Admin-Navigation',
            content: 'Der Admin-Bereich umfasst: Benutzerverwaltung (Benutzer, Rollen, Registrierungen), Schulstruktur (Bereiche, Räume, Familien), Module und Einstellungen (Sprache, Registrierung, Stunden, Feiertage), Design (Logo, Farben), Putz-Orga (Konfiguration, QR-Codes), Jahresabrechnung (Perioden, PDF-Export) und Fehlermeldungen.',
          },
        },
        users: {
          title: 'Benutzerverwaltung',
          manage: {
            title: 'Benutzer verwalten',
            content: 'Unter „Benutzerverwaltung" siehst du alle Nutzer mit Suchfunktion und Rollenfilter. Du kannst Benutzer aktivieren, deaktivieren oder löschen, deren Rollen ändern und Sonderrollen (Elternbeirat, PutzOrga) zuweisen.',
            content2: 'Beispiel: Um einen neuen Bereichs-Admin einzurichten, bearbeite den Benutzer, weise die Rolle „SECTION_ADMIN" zu und wähle unter Sonderrollen die zu verwaltenden Bereiche aus (z.B. „Grundstufe" und „Mittelstufe").',
          },
          roles: {
            title: 'Rollen & Sonderrollen',
            content: 'Fünf Hauptrollen: Superadmin (rot, Vollzugriff), Bereichsadmin (orange, Bereichsverwaltung), Lehrkraft (blau, Raumverwaltung), Elternteil (grün, Familienfunktionen) und Schüler (grau, eingeschränkt). Nutzer können mehrere Rollen haben und zwischen ihnen wechseln.',
            content2: 'Zusätzliche Sonderrollen: Elternbeirat (erweiterte Rechte für Termine, Formulare und Beiträge auf Bereichsebene) und PutzOrga (Zugriff auf die Putz-Konfiguration). Sonderrollen werden vom Bereichsadmin oder Superadmin vergeben.',
          },
          approval: {
            title: 'Registrierungen',
            content: 'Wenn „Neue Benutzer müssen freigeschaltet werden" in den Einstellungen aktiv ist, werden neue Registrierungen zunächst als inaktiv angelegt. Du siehst ausstehende Genehmigungen im Admin-Dashboard und kannst sie dort annehmen oder ablehnen. Diese Einstellung ist unter Administration → Einstellungen konfigurierbar.',
          },
        },
        structure: {
          title: 'Schulstruktur',
          sections: {
            title: 'Schulbereiche',
            content: 'Schulbereiche bilden die organisatorische Grundstruktur: z.B. Kinderhaus, Grundstufe, Mittelstufe, Oberstufe. Jeder Bereich hat einen Namen, eine Beschreibung und eine Sortierreihenfolge.',
            content2: 'Bereiche werden Räumen zugeordnet und bestimmen die Sichtbarkeit von Formularen (Bereichs-Scope) und Kalender-Events (Bereichs-Termine). Bereichs-Admins verwalten nur die ihnen explizit zugewiesenen Bereiche.',
          },
          rooms: {
            title: 'Räume',
            content: 'Erstelle und verwalte alle Räume der Schule. Ordne jeden Raum einem Schulbereich zu und setze Raumleitungen (LEADER) ein. Raumtypen: KLASSE (Klassenraum), GRUPPE (Arbeitsgruppe), PROJEKT und CUSTOM.',
            content2: 'KLASSE-Räume erhalten automatisch einen Standard-Ordner im Dateibereich. Du kannst Räume archivieren (Mitglieder können noch lesen, aber nicht mehr schreiben) und wiederherstellen. Die Beitrittspolitik (Offen/Anfrage/Einladung) wird pro Raum konfiguriert.',
          },
          families: {
            title: 'Familien',
            content: 'Überblick über alle Familienverbünde mit Namen, Mitgliedern und Stundenkontostände. Du kannst Familien bearbeiten, Mitglieder hinzufügen oder entfernen, und die Stunden-Befreiung aktivieren.',
            content2: 'Befreite Familien erscheinen nicht in der regulären Stundenbilanz (z.B. für Sonderfälle). Du kannst Familien auch deaktivieren — deaktivierte Familien bleiben erhalten, erscheinen aber nicht mehr in aktiven Listen. Bereichsadmins haben ebenfalls Zugriff auf die Familienverwaltung.',
          },
        },
        system: {
          title: 'Systemeinstellungen',
          modules: {
            title: 'Module',
            content: 'Aktiviere oder deaktiviere einzelne Module: Nachrichten, Dateien, Jobbörse, Putz-Orga, Kalender, Formulare, Fotobox und Fundgrube. Deaktivierte Module verschwinden aus der Navigation und die zugehörigen API-Endpunkte geben 404 zurück.',
            content2: 'Zusätzlich konfigurierst du hier die Kommunikationsregeln: Lehrer ↔ Eltern ist immer erlaubt. Eltern ↔ Eltern und Schüler ↔ Schüler Kommunikation kann einzeln freigeschaltet oder gesperrt werden.',
          },
          settings: {
            title: 'Einstellungen',
            content: 'Unter „Einstellungen" konfigurierst du: Mehrsprachigkeit (DE/EN an/aus), Standardsprache, ob neue Benutzer freigeschaltet werden müssen, ob Stundenbestätigung durch Verantwortliche erforderlich ist, Stundenziele (gesamt und Putzstunden), Bundesland für Feiertage und Schulferien-Zeiträume.',
          },
          theme: {
            title: 'Design',
            content: 'Unter „Design" passt du das Erscheinungsbild an: Schullogo hochladen (wird in Navigation und Login angezeigt) und Farbschema festlegen (Primärfarbe, Hintergrundfarben, Textfarben, Rahmenfarben). Eine Live-Vorschau zeigt die Änderungen sofort an.',
          },
          cleaning: {
            title: 'Putz-Organisation',
            content: 'Erstelle Putzaktionen — entweder wiederkehrend (Wochentag, Uhrzeit, max. Teilnehmer, Stundenwert) oder einmalig (spezifisches Datum). Im Datumswaehler werden Feiertage rot und Schulferien orange markiert.',
            content2: 'Putzaktionen erzeugen automatisch einen Kalender-Eintrag und einen Job in der Jobbörse. Du kannst QR-Codes für Putzorte generieren und drucken, den Check-in-Status überwachen und die Putz-Statistiken (Anwesenheit, Stunden pro Familie) einsehen.',
          },
          billing: {
            title: 'Jahresabrechnung',
            content: 'Erstelle Abrechnungsperioden (Jahr/Monat), generiere Berichte für alle Familien und exportiere sie als PDF. Geschlossene Perioden können nicht mehr verändert werden. Die Abrechnung zeigt pro Familie: reguläre Stunden, Putzstunden, Gesamtstunden, Soll und Saldo.',
          },
          errorReports: {
            title: 'Fehlermeldungen',
            content: 'Frontend-Fehler werden automatisch gemeldet und unter „Fehlermeldungen" mit Fingerprint-basierter Deduplizierung angezeigt. Du kannst den Status verwalten (Neu/Gemeldet/Behoben/Ignoriert) und optional GitHub-Issues aus Fehlermeldungen erstellen, wenn ein GitHub-Repository und Personal Access Token konfiguriert sind.',
          },
          privacy: {
            title: 'Datenschutz (DSGVO)',
            content: 'Verwalte Datenschutzerklärung und Nutzungsbedingungen unter „Datenschutz". Texte werden als HTML hinterlegt und unterstützen Platzhalter (z.B. SCHOOL_NAME, SCHOOL_ADDRESS in doppelten geschweiften Klammern). Nutzer müssen neue Versionen der Nutzungsbedingungen akzeptieren.',
            content2: 'Im DSGVO-Tab der Benutzerverwaltung kannst du personenbezogene Daten einzelner Nutzer exportieren und Kontoloeschungen mit 14-Tage-Frist einleiten. Das Datenzugriffs-Protokoll zeichnet alle Admin-Zugriffe auf Nutzerdaten auf (Art. 15 DSGVO).',
          },
        },
      },
    },
  },
  privacy: {
    title: 'Datenschutz',
    privacyPolicy: 'Datenschutzerklarung',
    termsOfService: 'Nutzungsbedingungen',
    noPrivacyPolicy: 'Es wurde noch keine Datenschutzerklarung hinterlegt.',
    noTermsConfigured: 'Es wurden noch keine Nutzungsbedingungen hinterlegt.',
    acceptTerms: 'Nutzungsbedingungen akzeptieren',
    termsAccepted: 'Nutzungsbedingungen akzeptiert',
    exportData: 'Meine Daten exportieren',
    exportDataDesc: 'Laden Sie alle Ihre personenbezogenen Daten als JSON-Datei herunter.',
    exportSuccess: 'Datenexport heruntergeladen',
    exportError: 'Fehler beim Exportieren der Daten',
    deleteAccount: 'Konto loeschen',
    deleteAccountDesc: 'Ihr Konto und alle zugehoerigen Daten werden nach einer 14-taegigen Frist unwiderruflich geloescht.',
    deleteConfirmTitle: 'Konto wirklich loeschen?',
    deleteConfirmMessage: 'Ihr Konto wird in 14 Tagen endgueltig geloescht. Sie koennen die Loeschung innerhalb dieser Frist jederzeit abbrechen.',
    deletionRequested: 'Kontoloeschung angefragt',
    deletionScheduled: 'Ihr Konto wird am {date} geloescht.',
    cancelDeletion: 'Loeschung abbrechen',
    deletionCancelled: 'Kontoloeschung abgebrochen',
    deletionError: 'Fehler bei der Kontoloeschung',
    cancelError: 'Fehler beim Abbrechen der Loeschung',
    viewPrivacyPolicy: 'Datenschutzerklarung anzeigen',
    adminPrivacy: 'Datenschutz',
    adminPrivacyDesc: 'Datenschutzerklarung, Nutzungsbedingungen und Aufbewahrungsfristen',
    policyText: 'Datenschutzerklarung (HTML)',
    policyTextHint: 'HTML-formatierter Text der Datenschutzerklarung',
    policyVersion: 'Version',
    termsText: 'Nutzungsbedingungen (HTML)',
    termsTextHint: 'HTML-formatierter Text der Nutzungsbedingungen',
    termsVersion: 'Version',
    retention: 'Aufbewahrungsfristen',
    retentionNotificationDays: 'Benachrichtigungen (Tage)',
    retentionNotificationDaysHint: 'Benachrichtigungen werden nach dieser Anzahl von Tagen automatisch geloescht.',
    retentionAuditDays: 'Audit-Log (Tage)',
    retentionAuditDaysHint: 'Audit-Log-Eintraege werden nach dieser Anzahl von Tagen automatisch geloescht.',
    saved: 'Datenschutz-Einstellungen gespeichert',
    termsNotAccepted: 'Bitte akzeptieren Sie die aktuellen Nutzungsbedingungen.',
    consent: 'Einwilligungen',
    photoConsent: 'Foto-Einwilligung',
    chatConsent: 'Chat-Einwilligung',
    consentGranted: 'Erteilt',
    consentRevoked: 'Widerrufen',
    pendingDeletions: 'Ausstehende Loeschungen',
    dataAccessLog: 'Datenzugriffs-Protokoll',
  },
  mentions: {
    noResults: 'Keine Benutzer gefunden',
  },
  twoFactor: {
    title: 'Zwei-Faktor-Authentifizierung',
    enable: '2FA aktivieren',
    disable: '2FA deaktivieren',
    enabled: '2FA aktiv',
    scanQr: 'QR-Code scannen',
    manualEntry: 'Manuell eingeben',
    enterCode: 'Code eingeben',
    verify: 'Bestätigen',
    recoveryCodes: 'Wiederherstellungscodes',
    recoveryCodesInfo: 'Speichern Sie diese Codes sicher. Jeder Code kann nur einmal verwendet werden.',
    recoveryCodesSaved: 'Ich habe die Codes gespeichert',
    enterPassword: 'Passwort eingeben',
    codeRequired: '2FA-Code erforderlich',
    invalidCode: 'Ungültiger Code',
    setupRequired: '2FA-Einrichtung erforderlich',
    setupRequiredDesc: 'Ihr Administrator verlangt die Einrichtung der Zwei-Faktor-Authentifizierung.',
    gracePeriod: 'Frist: {date}',
    disableConfirm: 'Zum Deaktivieren Ihr Passwort eingeben',
    setupSuccess: '2FA erfolgreich aktiviert',
    disableSuccess: '2FA wurde deaktiviert',
    modes: {
      DISABLED: 'Deaktiviert',
      OPTIONAL: 'Optional',
      MANDATORY: 'Pflicht',
    },
    adminMode: '2FA-Modus',
    adminModeHint: 'Bei "Pflicht" haben Nutzer 7 Tage Zeit zur Einrichtung.',
    graceDeadline: 'Frist für Pflicht-2FA: {date}',
  },
  directory: {
    title: 'Verzeichnis',
    tabUsers: 'Benutzer',
    tabFamilies: 'Familien',
    search: 'Name oder E-Mail suchen...',
    searchFamilies: 'Familie oder Mitglied suchen...',
    filterRole: 'Alle Rollen',
    filterSection: 'Alle Bereiche',
    filterRoom: 'Alle Räume',
    noResults: 'Keine Benutzer gefunden',
    noFamilies: 'Keine Familien gefunden',
    showingCount: '{count} Benutzer',
    memberCount: '{count} Mitglieder',
    startChat: 'Nachricht senden',
    sendMessage: 'Nachricht senden',
    roles: {
      SUPERADMIN: 'Superadmin',
      SECTION_ADMIN: 'Bereichsadmin',
      TEACHER: 'Pädagoge',
      PARENT: 'Elternteil',
      STUDENT: 'Schüler/in',
    },
  },
  search: {
    title: 'Suche',
    placeholder: 'Suchen...',
    noResults: 'Keine Ergebnisse gefunden',
    hint: 'Mindestens 2 Zeichen eingeben',
    shortcutHint: 'zum Suchen',
    filterAll: 'Alle',
    filterUsers: 'Benutzer',
    filterRooms: 'Raeume',
    filterPosts: 'Beitraege',
    filterEvents: 'Termine',
    filterFiles: 'Dateien',
    filterWiki: 'Wiki',
    filterTasks: 'Aufgaben',
    typeUser: 'Benutzer',
    typeRoom: 'Raeume',
    typePost: 'Beitraege',
    typeEvent: 'Termine',
    typeFile: 'Dateien',
    typeWiki: 'Wiki-Seiten',
    typeTask: 'Aufgaben',
  },
  csvImport: {
    title: 'CSV-Import',
    subtitle: 'Benutzer und Familien per CSV importieren',
    stepInstructions: 'Anleitung & Beispiel',
    stepUpload: 'Hochladen & Vorschau',
    stepImport: 'Import & Ergebnis',
    description: 'Mit dem CSV-Import koennen Sie viele Benutzer und Familien auf einmal anlegen. Das ist besonders nuetzlich zum Schuljahresbeginn, wenn hunderte Konten erstellt werden muessen.',
    formatTitle: 'CSV-Format',
    formatDescription: 'Die CSV-Datei muss folgende Spalten enthalten (durch Semikolon getrennt):',
    colEmail: 'E-Mail (erforderlich) - Die E-Mail-Adresse des Benutzers',
    colFirstName: 'Vorname (erforderlich) - Der Vorname',
    colLastName: 'Nachname (erforderlich) - Der Nachname',
    colRole: 'Rolle (erforderlich) - PARENT, STUDENT, TEACHER oder SECTION_ADMIN',
    colFamilyName: 'Familienname (optional) - Name des Familienverbunds. Benutzer mit gleichem Familiennamen werden automatisch derselben Familie zugeordnet',
    colFamilyRole: 'Familienrolle (optional, aber erforderlich wenn Familienname angegeben) - PARENT oder CHILD',
    colSectionSlug: 'Schulbereich (optional) - Kuerzel des Schulbereichs (z.B. krippe, kindergarten, grundstufe)',
    separatorHint: 'Als Trennzeichen wird Semikolon (;) verwendet, da deutsches Excel standardmaessig Semikolons nutzt. Tabulator und Komma werden ebenfalls unterstuetzt.',
    encodingHint: 'Die Datei muss in UTF-8 kodiert sein (fuer korrekte Umlaute).',
    passwordHint: 'Alle importierten Benutzer erhalten das Initialpasswort: changeme123. Bitte teilen Sie den Familien mit, dass sie das Passwort nach dem ersten Login aendern sollen.',
    downloadExample: 'Beispiel-CSV herunterladen',
    exampleTitle: 'Beispiel-Vorschau',
    uploadTitle: 'CSV-Datei hochladen',
    uploadHint: 'Waehlen Sie eine CSV-Datei aus oder ziehen Sie sie hierher.',
    uploadButton: 'Datei auswaehlen',
    previewTitle: 'Vorschau',
    previewSummary: '{users} Benutzer und {families} neue Familien werden erstellt',
    previewErrors: '{count} Fehler gefunden - bitte korrigieren Sie die CSV-Datei',
    previewValid: 'Alle Zeilen sind gueltig',
    colRow: 'Zeile',
    colName: 'Name',
    colStatus: 'Status',
    statusValid: 'Gueltig',
    statusError: 'Fehler',
    importButton: 'Import starten',
    importing: 'Importiere...',
    resultTitle: 'Import-Ergebnis',
    resultSuccess: 'Import erfolgreich abgeschlossen!',
    resultUsersCreated: '{count} Benutzer erstellt',
    resultFamiliesCreated: '{count} Familien erstellt',
    resultErrors: '{count} Fehler aufgetreten',
    resultNoErrors: 'Keine Fehler',
    startOver: 'Neuen Import starten',
    goToUsers: 'Zur Benutzerverwaltung',
    errorDetails: 'Fehlerdetails',
    dragDrop: 'CSV-Datei hierher ziehen oder klicken zum Auswaehlen',
    fileSelected: 'Datei ausgewaehlt: {name}',
    changeFile: 'Andere Datei waehlen',
    noFile: 'Keine Datei ausgewaehlt',
    validating: 'Validiere...',
  },
  poll: {
    createPoll: 'Umfrage erstellen',
    questionPlaceholder: 'Frage eingeben...',
    optionPlaceholder: 'Option {n}',
    addOption: 'Option hinzufügen',
    multipleChoice: 'Mehrfachauswahl',
    vote: 'Abstimmen',
    totalVotes: '{count} Stimme(n)',
    closed: 'Beendet',
    closePoll: 'Umfrage beenden',
    pollLabel: 'Umfrage',
  },
  profileFields: {
    title: 'Zusätzliche Profilfelder',
    subtitle: 'Benutzerdefinierte Felder für dein Profil',
    saved: 'Profilfelder gespeichert',
    noFields: 'Keine zusätzlichen Profilfelder konfiguriert',
    admin: {
      title: 'Profilfelder verwalten',
      subtitle: 'Benutzerdefinierte Felder für Benutzerprofile erstellen und verwalten',
      newField: 'Neues Feld',
      editField: 'Feld bearbeiten',
      fieldKey: 'Feldschlüssel',
      fieldKeyHint: 'Nur Kleinbuchstaben, Zahlen und Unterstriche (z.B. hobby, birthday)',
      labelDe: 'Bezeichnung (DE)',
      labelEn: 'Bezeichnung (EN)',
      fieldType: 'Feldtyp',
      options: 'Optionen',
      optionsHint: 'Kommagetrennte Optionen (nur für Auswahl-Typ)',
      required: 'Pflichtfeld',
      position: 'Position',
      active: 'Aktiv',
      noFields: 'Noch keine Profilfelder angelegt',
      deleteConfirm: 'Möchtest du dieses Profilfeld wirklich löschen? Alle Benutzerdaten für dieses Feld werden ebenfalls gelöscht.',
      created: 'Profilfeld erstellt',
      updated: 'Profilfeld aktualisiert',
      deleted: 'Profilfeld gelöscht',
    },
    fieldTypes: {
      TEXT: 'Text',
      DATE: 'Datum',
      SELECT: 'Auswahl',
      BOOLEAN: 'Ja/Nein',
    },
  },
  parentLetters: {
    title: 'Elternbriefe',
    preview: 'Vorschau',
    create: 'Neuen Brief erstellen',
    editLetter: 'Brief bearbeiten',
    noLetters: 'Keine Elternbriefe vorhanden.',
    noInbox: 'Keine Elternbriefe in Ihrem Posteingang.',
    tabs: {
      mine: 'Meine Briefe',
      inbox: 'Posteingang',
    },
    room: 'Raum (Klasse)',
    selectRoom: 'Klasse auswählen',
    letterTitle: 'Betreff',
    titlePlaceholder: 'z.B. Wandertag am 15. Mai',
    content: 'Inhalt',
    contentPlaceholder: 'Liebe {Anrede},\n\nwir möchten Sie informieren, dass ...\n\nMit freundlichen Grüßen,\n{LehrerName}',
    recipients: 'Empfänger',
    entireClass: 'Gesamte Klasse',
    selectStudents: 'Schüler/innen auswählen',
    sendDate: 'Absendedatum',
    sendDateHint: 'Leer lassen für sofortigen Versand',
    deadline: 'Bestätigungsfrist',
    deadlineHint: 'Bis wann muss bestätigt werden?',
    reminderDays: 'Erinnerung (Tage vorher)',
    reminderDaysHint: 'Automatische Erinnerung X Tage vor Frist',
    saveDraft: 'Als Entwurf speichern',
    send: 'Brief senden',
    close: 'Brief schließen',
    saved: 'Brief gespeichert',
    sent: 'Brief gesendet',
    closed: 'Brief geschlossen',
    deleted: 'Brief gelöscht',
    confirmed: 'Bestätigung erfolgreich',
    confirmAction: 'Bestätigen',
    confirmPrompt: 'Hiermit bestätige ich den Erhalt und die Kenntnisnahme dieses Elternbriefs.',
    allConfirmed: 'Alle Bestätigungen eingegangen',
    pendingConfirmation: 'Bestätigung ausstehend',
    statuses: {
      DRAFT: 'Entwurf',
      SCHEDULED: 'Geplant',
      SENT: 'Gesendet',
      CLOSED: 'Geschlossen',
    },
    recipientStatuses: {
      OPEN: 'Offen',
      READ: 'Gelesen',
      CONFIRMED: 'Bestätigt',
    },
    tracking: {
      title: 'Bestätigungsstatus',
      student: 'Schüler/in',
      parent: 'Elternteil',
      family: 'Familie',
      status: 'Status',
      readAt: 'Gelesen am',
      confirmedAt: 'Bestätigt am',
      confirmedBy: 'Bestätigt von',
      reminderSent: 'Erinnerung gesendet',
      total: 'Gesamt',
      read: 'Gelesen',
      confirmedCount: 'Bestätigt',
    },
    recipientTable: {
      total: 'Gesamt',
      read: 'Gelesen',
      confirmed: 'Bestätigt',
      empty: 'Keine Empfänger',
      student: 'Schüler/in',
      parent: 'Elternteil',
      family: 'Familie',
      status: 'Status',
      readAt: 'Gelesen am',
      confirmedAt: 'Bestätigt am',
      reminderSent: 'Erinnerung gesendet',
    },
    confirm: 'Bestätigen',
    variables: {
      title: 'Verfügbare Platzhalter',
      helpTitle: 'Verfügbare Variablen',
      helpSubtitle: 'Diese Platzhalter werden automatisch ersetzt',
      Familie: 'Familienname',
      NameKind: 'Vorname des Kindes',
      Anrede: 'Anrede der Eltern',
      LehrerName: 'Name des Lehrers/Erstellers',
      insert: 'Einfügen',
      copied: 'In Zwischenablage kopiert',
    },
    config: {
      title: 'Elternbrief-Einstellungen',
      letterhead: 'Briefkopf',
      uploadLetterhead: 'Briefkopf hochladen',
      removeLetterhead: 'Briefkopf entfernen',
      signature: 'Signatur-Vorlage',
      signaturePlaceholder: 'Mit freundlichen Grüßen,\n{LehrerName}',
      reminderDays: 'Standard-Erinnerung (Tage)',
      saved: 'Einstellungen gespeichert',
      global: 'Schulweite Einstellungen',
      section: 'Bereichs-Einstellungen',
    },
    notification: {
      newLetter: 'Neuer Elternbrief',
      newLetterMessage: 'Sie haben einen neuen Elternbrief erhalten: {title}',
      reminder: 'Erinnerung: Elternbrief bestätigen',
      reminderMessage: 'Bitte bestätigen Sie den Elternbrief "{title}" bis zum {deadline}.',
    },
    progress: '{confirmed} von {total} bestätigt',
    deadline_info: 'Frist: {date}',
    downloadPdf: 'Brief als PDF',
    downloadTracking: 'Rücklauf-Liste als PDF',
    confirmSuccess: 'Bestätigung erfolgreich',
    confirmProgress: 'Bestätigungsfortschritt',
    confirmationRequired: 'Bestätigung erforderlich',
    confirmationHint: 'Bitte bestätigen Sie den Erhalt dieses Elternbriefs für jedes Ihrer Kinder.',
    recipientsTitle: 'Empfänger-Übersicht',
    notFound: 'Elternbrief nicht gefunden.',
    newLetter: 'Neuer Brief',
    createdAt: 'Erstellt am',
    form: {
      room: 'Raum (Klasse)',
      selectRoom: 'Klasse auswählen',
      noKlasseRooms: 'Sie sind in keinem Klassenraum als Leitung eingetragen.',
      title: 'Betreff',
      titlePlaceholder: 'z.B. Wandertag am 15. Mai',
      content: 'Inhalt',
      contentPlaceholder: 'Liebe {Anrede},\n\nwir möchten Sie informieren, dass ...\n\nMit freundlichen Grüßen,\n{LehrerName}',
      recipients: 'Empfänger',
      sendToAll: 'An alle Schüler/innen senden',
      selectStudents: 'Schüler/innen auswählen',
      selectStudentsPlaceholder: 'Schüler/innen auswählen...',
      sendDate: 'Absendedatum',
      deadline: 'Bestätigungsfrist',
      reminderDays: 'Erinnerung (Tage vorher)',
      reminderDaysHint: 'Automatische Erinnerung X Tage vor Frist',
      optional: 'Optional',
    },
    stats: {
      active: 'Aktive Briefe',
      confirmRate: 'Rücklaufquote',
      overdue: 'Überfällig',
      daysLeft: 'Tage',
    },
    attachments: {
      title: 'Anhänge',
      upload: 'Dateien anhängen',
      uploadHint: 'Max. 5 Dateien, je max. 10 MB',
      delete: 'Anhang entfernen',
      deleted: 'Anhang entfernt',
      uploaded: 'Dateien hochgeladen',
      noAttachments: 'Keine Anhänge',
    },
  },
  footer: {
    deployed: 'Deployed',
  },
}
